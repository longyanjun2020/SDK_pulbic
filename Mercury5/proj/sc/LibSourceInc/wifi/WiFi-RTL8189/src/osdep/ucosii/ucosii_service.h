...
#if (defined(CONFIG_PLATFORM_MSTAR_MSC8328D) && defined(CONFIG_HIGH_TP))
...
typedef struct timer_list {
	OS_TMR *pucosii_timer;
	unsigned char timer_name[TMR_NAME_SIZE];
	TIMER_FUN callback;
	void *context;
	unsigned char opt;//OS_TMR_OPT_ONE_SHOT or OS_TMR_OPT_ONE_SHOT
} _timer;
#if 0
struct	__queue	{
