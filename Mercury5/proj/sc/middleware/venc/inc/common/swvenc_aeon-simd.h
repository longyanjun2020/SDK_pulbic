...
#define MV_LDV(vD,rA,imm16)                 \
 {                                          \
    const int *p0 = (const int*)((void*)((const char*)rA+imm16));      \
    const int *p1 = (const int*)((void*)((const char*)rA+imm16+4));    \
    __asm__ volatile ("mv.ldv\t" #vD ",%1(%0);\t#__ASM__" \
        :                                   \
        : "r"(rA), "K"(imm16), "m"(*p0), "m"(*p1) \
    ); }                //ToDo: check 3-LSB is zero

#define MV_LDW(vD,rA,imm16)                 \
 {                                          \
    const int *p0 = (const int*)((void*)((const char*)rA+imm16));      \
    __asm__ volatile ("mv.ldw\t" #vD ",%1(%0);\t#__ASM__" \
        :                                   \
        : "r"(rA), "K"(imm16), "m"(*p0)     \
    ); }                //ToDo: check 2-LSB is zero

#define MV_SDV(rA,imm16,vSRC)               \
 {                                          \
    int *p0 = (int*)((void*)((char*)rA+imm16));      \
    int *p1 = (int*)((void*)((char*)rA+imm16+4));    \
    __asm__ volatile ("mv.sdv\t%3(%2)," #vSRC ";\t#__ASM__" \
        : "=m"(*p0), "=m"(*p1)              \
        : "r"(rA), "K"(imm16)               \
        : "memory"                          \
    ); }                //ToDo: check 3-LSB is zero

#define MV_SDW(rA,imm16,vSRC)               \
 {                                          \
    int *p0 = (int*)((void*)((char*)rA+imm16));      \
    __asm__ volatile ("mv.sdv\t%2(%1)," #vSRC ";\t#__ASM__" \
        : "=m"(*p0)                         \
        : "r"(rA), "K"(imm16)               \
    ); }                //ToDo: check 2-LSB is zero
