...
static inline u8 * wpa_sm_alloc_wai(struct wpa_sm *sm, u8 type,
                      const void *data, u16 data_len,
                      size_t *msg_len, void **data_pos)
{
    WPA_ASSERT(sm->ctx->alloc_wai);
    return sm->ctx->alloc_wai(sm->ctx->ctx, type, data, data_len,
                    msg_len, data_pos);
}
...
void wpa_wai_key_send(struct wpa_sm *sm, const u8 *dest, u16 proto, u8 *msg, size_t msg_len);
