...
struct wpa_driver_ops {
	 ...
	 */
	int (*set_param)(void *priv, const void *param);
...
typedef enum wpa_event_type {
	...
	EVENT_ASSOC,
	/**
	 * EVENT_CONNECT_COMPLETE - Connection completed
	 *
	 * The EVENT happened depend on what security policy is adopted. For OPEN
	 * shared key security, the connection is completed once station is associated
	 * with AP. However, the connection is not completed for WPA and WPA2 until
	 * station has complete key negotiation.
	 *
	 */
	EVENT_CONNECT_COMPLETE,
	/**
	 * EVENT_ROAMING - Roaming Connection completed
	 *
	 * The EVENT happened when connection is disconnected and then re-connected
	 * successfully. This event is added for driver with roaming capability. The
	 * wpa_supplicant rely on this event to distinguish connection type and make
	 * proper response.
	 */
	EVENT_ROAMING,
	/**
	...
	/**
	 * EVENT_AUTH_FAIL - Authentication Fail
	 *
	 * This event is used as connection cannot be setup within specific
	 * time. The failed reasons may included wrong configuration or
	 * password.
	 */
	EVENT_AUTH_FAIL,
	/**
	 * EVENT_SHARED_KEY_FAIL - Authentication Shared-key Fail
	 *
	 * This event is used for shared-key authentication. When wrong password
	 * is used for shared-key authentication, the M3 should be able to notify
	 * the B2 that wrong password is used.
	 */
	EVENT_SHARED_KEY_FAIL
