...
#if 0 //kc
#define CERT_TIME_Z_LEN 14 //GeneralizedTime: YYYYMMDDHHMMSSZ

#define CERT_STS_TIME_INVALID        1
#define CERT_STS_X509_FORMAT_ERROR   2
...
#include "akm_cred.h"
#endif
enum X509_VERSION { X509_CERT_V1 = 0, X509_CERT_V2 = 1, X509_CERT_V3 = 2 };
struct x509_certificate {
	struct x509_certificate *next;
	enum X509_VERSION version;
	unsigned long serial_number;
    ...
    u8 notbefore[CERT_TIME_Z_LEN]; //kc
    u8 notafter[CERT_TIME_Z_LEN]; //kc
...
#define X509_KEY_USAGE_DECIPHER_ONLY		(1 << 8)
	...
	size_t tbs_cert_len;
    const u8 *tbs_issuer_start;
    size_t tbs_issuer_len;
    const u8 *tbs_subject_start;
    size_t tbs_subject_len;
    const u8 *tbs_serial_start;
    size_t tbs_serial_len;
...
void x509_free_name(struct x509_name *name); //kc
