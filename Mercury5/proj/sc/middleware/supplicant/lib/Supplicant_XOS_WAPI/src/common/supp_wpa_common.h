...
#define WAPI_SELECTOR_LEN 4         //WAPI support
...
struct wai_ukey_resp {
    ...
    u8 asue_chalenge[32];
    u8 ae_chalenge[32];
    u8 ie[1];        // WAPI ASUE IE
    u8 *hmac;        // Message Authentication Code
} STRUCT_PACKED;

struct wai_ukey_conf {
    u8 flag;
    u8 bkid[16];
    u8 uskid;
    u8 addid[12];
...
union wai_key {
    struct wai_ukey_req     ukey_req;
    struct wai_ukey_resp    ukey_resp;
    struct wai_ukey_conf    ukey_conf;
    struct wai_mkey_annou   mkey_annou;
    struct wai_mkey_resp    mkey_resp;
};
/**
 ...
 */
struct wpa_usk {
	u8 uek[16]; /* WAI-KEY Unicast Encryption Key (UEK) */
	u8 uck[16]; /* WAI-KEY Unicast Check Key (UCK) */
	u8 mak[16]; /* Message Authentication Key (MAK) */
    u8 kek[16]; /* Key Encryption Key (KEK) */
    u8 challenge[32]; /* Challenge */
} STRUCT_PACKED;

struct wpa_msk {
    u8 mek[16]; /* WAI-KEY Multicast Encryption Key (MEK) */
    u8 mck[16]; /* WAI-KEY Multicast Check Key (MCK) */
} STRUCT_PACKED;
...
int wpa_eapol_key_mic(const u8 *key, int ver, const u8 *buf, size_t len,
		      u8 *mic);
int wpa_wai_key_mic(const u8 *key, const u8 *buf, size_t len, u8 *mic);
void wpa_pmk_to_ptk(const u8 *pmk, size_t pmk_len, const char *label,
		    const u8 *addr1, const u8 *addr2,
		    const u8 *nonce1, const u8 *nonce2,
		    u8 *ptk, size_t ptk_len, int use_sha256);

void wapi_bk_to_usk(const u8 *bk, size_t bk_len, const char *label,
		    const u8 *addr1, const u8 *addr2,
		    const u8 *nonce1, const u8 *nonce2,
		    u8 *usk, size_t usk_len);
#ifdef CONFIG_IEEE80211R
...
int wpa_parse_wapi_ie_wapi(const u8 *rsn_ie, size_t rsn_ie_len,
			 struct wpa_ie_data *data);
u32 freq_to_chan(u32 freq);
u32 chan_to_freq(u32 chan);
