...
#include "supp_defs.h"
#include "supp_eap_config.h"
#include "wai_config.h"
...
struct wpa_ssid {
	...
	int scan_ssid;
	/**
	 * hidden_ssid - Connect to hidden SSID
	 *
	 * command WiFi driver for hidden SSID connection
	 */
	int hidden_ssid;
...
#ifdef WAPI_WAIAKM
#define WAIAKM_FLAG_REQUIRE_KEY_UNICAST BIT(0)
#define WAIAKM_FLAG_REQUIRE_KEY_BROADCAST BIT(1)
    /**
     * waiakm_flags - Bit field of WAI auth and key management options (EAPOL_FLAG_*)
     */
    int waiakm_flags;

    /**
     * wai - WAI peer configuration for this network
     */
    struct wai_peer_config wai;
#endif /* WAPI_WAIAKM */
