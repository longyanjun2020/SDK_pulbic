...
#define SMULL_WB(Rm,Rs,Rd) arm9e_dsp_smulwb(Rd,Rm,Rs)
#define SMACL_WB(Rm,Rs,Rd) arm9e_dsp_smlawb(Rd,Rm,Rs,Rd)

#define SMSBL_WB(Rm,Rs,Rd) \
{  \
   int _Rm_ = -Rm; \
   arm9e_dsp_smlawb(Rd,_Rm_,Rs,Rd); \
}

#define SMULL_HIGH(Rm,Rs,Rd)  arm9e_smull_hi(Rd,Rm,Rs)

#define SMULL_MACR_n(Rm,Rs,Rd,n) \
{ \
   unsigned int Rdlo; \
   int Rdhi; \
