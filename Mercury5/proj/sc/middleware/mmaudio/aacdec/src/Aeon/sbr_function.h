...
//unsigned int aac_sbr_bit_read(int len);
//unsigned int aac_sbr_1bit_read(void);
unsigned int sbr_bit_read(buffer_bs *bs,int len);
unsigned int sbr_bit_view(buffer_bs *bs,int len);
static INLINE unsigned int aac_sbr_bit_read(int len)
{
   return sbr_bit_read(&pad->sbr_bs,len);
}

static INLINE unsigned int aac_sbr_1bit_read(void)
{
   return sbr_bit_read(&pad->sbr_bs,1);
}


static INLINE unsigned int aac_sbr_get_processed_bits(void)
{
	return pad->sbr_bs.bits_consumed;
}


static INLINE void aac_sbr_bit_flush(int len)
{
...
void sbrGetEnvelope(sbr_info *sbr, int ch);
void sbrGetNoiseFloorData(sbr_info *sbr, int ch);
void hf_generation(sbr_info *sbr, qmf_t X[MAX_NTSRHFG][64], sbr_ch_info *sbr_ch,int ch);
void GQ_addr_reset(sbr_info *sbr, int ch);
unsigned int hf_adjustment(sbr_info *sbr, qmf_t Xsbr[MAX_NTSRHFG][64], int ch);
unsigned int estimate_current_envelope(sbr_info *sbr,
                                         qmf_t Xsbr[MAX_NTSRHFG][64], sbr_ch_info *sbr_ch);
void calculate_gain(sbr_info *sbr, sbr_hfadj_info *adj, int ch);
void hf_assembly(sbr_info *sbr, sbr_hfadj_info *adj, qmf_t Xsbr[MAX_NTSRHFG][64], sbr_ch_info *sbr_ch);
...
void qmfa_init(qmfa_info *qmfa, int qmfa_index);

void qmfs_init(qmfs_info *qmfs, int channels, int index);

void qmfs_end(qmfs_info *qmfs);

void sbr_qmf_analysis_32(sbr_info *sbr, qmfa_info *qmfa, const int *input,
                         qmf_t X[MAX_NTSRHFG][64], int offset, int kx);
...
sbr_info *sbrDecodeInit(int elementID, unsigned int sample_rate, int downSampledSBR,int ele);
void sbrDecodeEnd(sbr_info *sbr);
void sbrReset(sbr_info *sbr);
unsigned int sbr_extension_data(sbr_info *sbr, int cnt, int psResetFlag, int elementID,int extensionType);
...
unsigned int sbrDecodeSingleFrame(sbr_info *sbr, int *pcm_in,
                             int just_seeked, int downSampledSBR);
#if defined(PS_DEC)
unsigned int sbrDecodeSingleFramePS(sbr_info *sbr, int *left_channel, int *right_channel,
                               int just_seeked, int downSampledSBR);

/* ps_syntax.c */
int ReadPsData(ps_info *ps, int *header, int *nr_bits);

/* ps_dec.c */
ps_info *ps_init(unsigned char sr_index);
void ps_decode(ps_info *ps, qmf_t X_left[38][64], qmf_t X_right[38][64], int *stack);
...
int applySBR(sbr_info *sbr, sbr_bitstream *sbr_bs_info, int ch, int *numChannels,int SbrFrameOK);
...
int calc_sbr_crc(int crc_check,int len);
