...
Word32 Inv_sqrt_e (      /* (o) : output value   (range: 0<=val<1)            */
    Word32 L_x           /* (i) : input value    (range: 0<=val<=7fffffff)    */
);



Word32 Pow2_e (          /* (o) : result       (range: 0<=val<=0x7fffffff)    */
    Word16 exponent,   /* (i) : Integer part.      (range: 0<=val<=30)      */
    Word16 fraction    /* (i) : Fractional part.  (range: 0.0<=val<1.0)     */
);


void gc_pred_reset_e (gc_predState *st);



Word32 sqrt_l_exp_e (/* o : output value,                        Q31 */
