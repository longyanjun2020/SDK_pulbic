/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: ra_depack_internal.h,v 1.3 2005/04/29 17:42:44 hubbe Exp $
 * 
...
typedef struct ra_substream_hdr_struct
{
    ...
    UINT32      dBlockDuration;        /* Duration in ms of audio "block" */
...
HX_RESULT ra_depacki_add_packet(ra_depack_internal* pInt,
                                rm_packet*          pPacket, int *RMlbr_Block_Count, int *RMlbr_Sub_Stream);
...
HX_RESULT ra_get_frame_count(ra_depack_internal* pInt, int *RMlbr_Frame_Count, int RMlbr_Sub_Stream);
HX_RESULT ra_depacki_deinterleave_send(ra_depack_internal* pInt, UINT32 ulSubStream);
HX_RESULT ra_depacki_deinterleave_send2(ra_depack_internal* pInt, UINT32 ulSubStream, ra_block* pBlock, int block_offset);
