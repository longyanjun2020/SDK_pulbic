/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: ra_depack.h,v 1.3 2005/04/29 17:42:06 hubbe Exp $
 * 
 ...
 */
void rm_depack_clear_flag(void);
HX_RESULT ra_depack_add_packet(ra_depack* pDepack, rm_packet* pPacket, int  *RMlbr_Block_Count, int *RMlbr_Sub_Stream);
HX_RESULT ra_depack_deinterleave_packet(ra_depack* pDepack, int RMlbr_Sub_Stream);
HX_RESULT ra_depack_now_packet(ra_depack* pDepack, int RMlbr_Sub_Stream, int block_offset, int frame_offset, int clear_bufflag);
HX_RESULT ra_depack_get_frame_count(ra_depack* pDepack, int  RMlbr_Block_Count, int RMlbr_Sub_Stream, int *RMlbr_Frame_Count);
/*
