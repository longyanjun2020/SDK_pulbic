 ...
 *           This function must be called first before you call any camera interface function.
 *
 * @param camID		        : Sensor device id.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 ...
 *
 * @return ESL_CAM_OK    : Open success.
 * @return ESL_CAM_FAIL  : Open fail.
 */
esl_CamErrCode_e esl_CamOpen(esl_CamID_e camID, esl_CamCb_t *pCamCb);


/**
 * @brief Use to free, re-setting software content or control hardware settings - LCD setting, GPIO control...
 *           This function is paired with esl_CamOpen, it must be called after you finised all of camera fuctions.
 *
 * @param camID		        : Sensor device id.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK    : Close success.
 * @return ESL_CAM_FAIL  : Close fail.
 */
esl_CamErrCode_e esl_CamClose(esl_CamID_e camID, esl_CamCb_t *pCamCb);


/**
 * @brief Set the configurations for JPEG capture or Video recording mode -  Like storage folder name.
 *
 * @param camMedium				 : Set the medium information.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK    : Set path success.
 * @return ESL_CAM_FAIL  : Set path fail.
 */
esl_CamErrCode_e esl_CamSetConfig(esl_CamMedium_t camMedium, esl_CamCb_t *pCamCb);


/**
 * @brief Get storage path of JPEG capture or Video recording.
 *
 * @param  pPath      : The buffer to save file name.
 * @param  pPathLen : Application need to assign the max buffer size and then it will return file name length.
 *
 * @return ESL_CAM_OK    : Get path success.
 * @return ESL_CAM_FAIL  : Get path fail.
 */
esl_CamErrCode_e esl_CamGetConfig(u8 *pPath, u16 *pPathLen);


/**
 * @brief Set camera attributions - AE, AWB, Zoom, EV, Image effect, Record size...
 *
 * @param camAttr            : Attribution for AE, AWB, Zoom, EV, Image effect, Record size...
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK    : Set attribution success.
 * @return ESL_CAM_FAIL  : Set attribution fail.
 */
esl_CamErrCode_e esl_CamSetAttribution(esl_CamAttr_t camAttr, esl_CamCb_t *pCamCb);


/**
 * @brief Get camera attributions value - AE, AWB, Zoom, EV, Image effect, Record size...
 *
 * @param pCamAttr          : Attribution poniter for saving the value of  AE, AWB, Zoom, EV, Image effect, Record...
 *
 * @return ESL_CAM_OK    : Get attribution success.
 * @return ESL_CAM_FAIL  : Get attribution fail.
 */
esl_CamErrCode_e esl_CamGetAttribution(esl_CamAttr_t *pCamAttr);


/**
 * @brief Start or Stop camera preview mode.
 *
 * @param camPreview       : ESL_CAM_PREVIEW_ON: start prview, ESL_CAM_PREVIEW_OFF: stop preview.
 * @param camDispRegion  : Set display property - display area, rotation..., this parameter is used when power off.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Power On or Off success.
 * @return ESL_CAM_FAIL   : Power On or Off fail.
 */
esl_CamErrCode_e esl_CamPreview(esl_CamPreview_e camPreview, esl_CamDispRegion_t camDispRegion, esl_CamCb_t *pCamCb);


/**
 * @brief Camera sanpshot. Format: JPEG Only.
 *
 * @param camJpegSetting : Property setting - JPEG or Video recording selection, Quality...
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Capture success.
 * @return ESL_CAM_FAIL   : Capture fail.
 */
esl_CamErrCode_e esl_CamCapture(esl_CamJpegSetting_t camJpegSetting, esl_CamCb_t *pCamCb);


/**
 * @brief Camera capture stop.
 *
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Capture stop success.
 * @return ESL_CAM_FAIL   : Capture stop fail.
 */
esl_CamErrCode_e esl_CamCaptureStop(esl_CamCb_t *pCamCb);


/**
 * @brief Start QR Code recording.
 *
 * @param camQRCSetting : Property setting - QR code recording time.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Recording start success.
 * @return ESL_CAM_FAIL   : Recording start fail.
 */
esl_CamErrCode_e esl_CamQRCStart(esl_CamQRCSetting_t camQRCSetting, esl_CamCb_t *pCamCb);


/**
 * @brief Stop QR Code recording .
 *
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Recording stop success.
 * @return ESL_CAM_FAIL   : Recording stop fail.
 */
esl_CamErrCode_e esl_CamQRCStop(esl_CamCb_t *pCamCb);


/**
 * @brief Camcoder video record. Format: H.263 Only.
 *
 * @param camVideoSetting : Property setting - JPEG or Video recording selection, resolution, quality...
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Record start success.
 * @return ESL_CAM_FAIL   : Record start fail.
 */
esl_CamErrCode_e esl_CamRecordStart(esl_CamVdoSetting_t camVdoSetting, esl_CamCb_t *pCamCb);


/**
 * @brief Camera video record stop. Format: H.263 Only.
 *
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Record stop success.
 * @return ESL_CAM_FAIL   : Record stop fail.
 */
esl_CamErrCode_e esl_CamRecordStop(esl_CamCb_t *pCamCb);


/**
 * @brief Camera video deferring save. Format: H.263 Only.
 *
 * @param pCamCb        : Call back structure, include sender ID, userValue and call back function.
 *                        It uses to send response message that provide by application layer(MMI)
 *                        to inform application layer(MMI) the process in EMA is success or fail.
 ...
 * @return ESL_CAM_FAIL : Record stop fail.
 */
esl_CamErrCode_e esl_CamRecordSave(esl_CamRecSaveInfo_t camRecSaveInfo, esl_CamCb_t *pCamCb);


/**
 * @brief Get the file name, after JPEG capture.
 *
 * @param pCamFileName   : File name lengthe and file name buffer, MMI should release file name buffer after receiving response msg.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Get file name success.
 * @return ESL_CAM_FAIL   : Get file name fail.
 */
esl_CamErrCode_e esl_CamGetFileName(esl_CamFileName_t *pCamFileName, esl_CamCb_t *pCamCb);


/**
 * @brief Get the image after JPEG capture.
 *
 * @param pCamImg  : Jepg image length and buffer, MMI should release image buffer (pCamImg->pImgOut) after receiving response msg.
 * @param pCamCb   : Call back structure, include sender ID, userValue and call back function.
 *                   It uses to send response message that provide by application layer(MMI)
 *                   to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Get image success.
 * @return ESL_CAM_FAIL   : Get image fail.
 */
esl_CamErrCode_e esl_CamGetImage(esl_CamImg_t *pCamImg, esl_CamCb_t *pCamCb);


/**
 * @brief Get QR code data.
 *
 * @param pCamGetQRC   : Getting QR Code length or data, MMI should release the buffer (pCamGetQRC->pCamQRCBuf) after receiving response msg.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Get QR Code success.
 * @return ESL_CAM_FAIL   : Get QR Code fail.
 */
esl_CamErrCode_e esl_CamGetQRC(esl_CamGetQRC_t *pCamGetQRC, esl_CamCb_t *pCamCb);


/**
 * @brief Set the camera shutter sound.
 *
 ...
 * @return ESL_CAM_FAIL  : Set path fail.
 */
esl_CamErrCode_e esl_CamSetShutterSound(u8 isEnable, esl_CamSound_t camSound, esl_CamCb_t *pCamCb);


/**
 * @brief Set the camera sticker picture.
 *
 * @param isEnable         : 0:Sticker disable, 1:Sticker enable
 * @param *pSticker        : Sticker image buffer.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK    : Set sticker success.
 * @return ESL_CAM_FAIL  : Set stikcer fail.
 */
esl_CamErrCode_e esl_CamSetSticker(u8 isEnable, u8 *pSticker, esl_CamCb_t *pCamCb);


/**
 * @brief Get the camera status of sensor, device and power.
 *
 * @param camStatus         : Status selection. ESL_CAM_STATUS_SETTING or ESL_CAM_STATUS_COLOR.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Get status success.
 * @return ESL_CAM_FAIL   : Get status fail.
 */
esl_CamErrCode_e esl_CamGetStatus(esl_CamStatus_e camStatus, esl_CamCb_t *pCamCb);


/**
 * @brief Get the capture file number after snapshot.
 *
 * @param pCamNum   : Capture file number.
 *
 * @return ESL_CAM_OK     : Get image success.
 * @return ESL_CAM_FAIL   : Get image fail.
 */
esl_CamErrCode_e esl_CamGetCaptureNum(u16 *pCamFileNum);

esl_CamErrCode_e esl_CamGetCaptureQuota(esl_CamGetQuota_t settings,esl_CamCb_t *pCamCb);


/**
 * @brief Register notification for camera capturing and video recording
 *
 * @param camNotifyType   : The type of register notification.
 * @param camNotifyCb      : Notification call back to inform MMI when Multimedia task receivd notification.
 * @param pCamCb           : Call back structure, include sender ID, userValue and call back function.
 *                                       It uses to send response message that provide by application layer(MMI)
 *                                       to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Register success.
 * @return ESL_CAM_FAIL   : Register fail.
 */
esl_CamErrCode_e esl_CamRegNotify(esl_CamNotifyType_e camNotifyType, esl_CamNotifyCb_t camNotifyCb, esl_CamCb_t *pCamCb);


/**
 * @brief Get the camera zoom information. [Cooper add]
 *
 * @param camZoom  : Property setting - camera information included capture size, rotate 90 degree.
 * @param pCamCb   : Call back structure, include sender ID, userValue and call back function.
 *                   It uses to send response message that provide by application layer(MMI)
 *                   to inform application layer(MMI) the process in EMA is success or fail.
 *
 * @return ESL_CAM_OK     : Get zoom status success.
 * @return ESL_CAM_FAIL   : Get zoom status fail.
 */
esl_CamErrCode_e esl_CamGetZoomInfo(esl_CamGetZoomInfo_t camZoomInfo, esl_CamCb_t *pCamCb);
esl_CamErrCode_e esl_CamMotionDetectSwitchReq(esl_CamID_e camID, u32 isEnable, esl_CamCb_t *pCamCb);
