...
typedef struct tagMP4Track {
    ...
    uint32_t        m_next_chunkFilePos;
    uint32_t        m_next_SPC; //next samples per chunk

    uint32_t       *m_stsz_offset; //
    ...
    uint32_t**      m_pChunkOffsetProperty;
    ...
    uint32_t*       m_pCttsSampleOffsetProperty;
...
MP4TRACK  *mxMP4TracksGetById(MP4TRACKARRAY *pArray, MP4TrackId trackId);
...
//-----------------------------------------------------------------------------
// Tracks Array API
MP4TRACK *CreateDmxTrack(void *ctx, struct tagMP4File *pfile, struct tagATOM *pTrack);
#define MP4TracksGetSize(pArray) ((pArray)->m_totalnum)
MP4TRACK  *MP4TracksGet(MP4TRACKARRAY *pArray, uint32_t index);
MP4TRACK  *MP4TracksGetById(MP4TRACKARRAY *pArray, MP4TrackId trackId);
