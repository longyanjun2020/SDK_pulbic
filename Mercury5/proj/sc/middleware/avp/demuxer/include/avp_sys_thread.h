...
do { \
    ...
    cyg_semaphore_init(&(t)->semaphore, 0); \
} while (0)

#define sys_thread_exit(t) do { \
    cyg_semaphore_post(&(t)->semaphore); \
    cyg_thread_exit(); \
} while (0)

#define sys_thread_join(t) do { \
    cyg_semaphore_wait(&(t)->semaphore); \
    cyg_semaphore_destroy(&(t)->semaphore); \
    cyg_thread_delete((t)->handle); \
} while (0)
...
do { \
    ...
    pthread_create(&(t)->handle, NULL, thread_routine, thread_data); \
} while (0)

#define sys_thread_exit(t) pthread_exit(NULL)

#define sys_thread_join(t) pthread_join((t)->handle, NULL)
