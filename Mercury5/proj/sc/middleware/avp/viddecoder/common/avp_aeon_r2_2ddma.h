...
//#define R2_BASE_ADDR 0xA0000000 //MMP off setting.
#define R2_BASE_ADDR 0xFFFF0000 //MMP on setting
#define OFFSET 0x40
#define R2_SWITCH_BANK (R2_BASE_ADDR + ((OFFSET+0x2C)<<2))
#define REG_VQM_BANK_SWITCH (1<<4)
#define REG_VQM_OWENER_SEL  (1<<3)

#define REG_VQMEM_EN        (R2_BASE_ADDR + ((OFFSET+0x18)<<2))
#define REG_VQMEM_SPACE_EN  (1<<14)

#define REG_DQMEM_BASE_LOW  (R2_BASE_ADDR + ((OFFSET+0x0D)<<2))
#define REG_DQMEM_BASE_HIGH (R2_BASE_ADDR + ((OFFSET+0x0E)<<2))
#define REG_DQMEM_MASK_LOW  (R2_BASE_ADDR + ((OFFSET+0x0F)<<2))
#define REG_DQMEM_MASK_HIGH (R2_BASE_ADDR + ((OFFSET+0x10)<<2))

#define REG_VQMEM_BASE_LOW  (R2_BASE_ADDR + ((OFFSET+0x27)<<2))
#define REG_VQMEM_BASE_HIGH (R2_BASE_ADDR + ((OFFSET+0x28)<<2))
#define REG_VQMEM_MASK_LOW  (R2_BASE_ADDR + ((OFFSET+0x29)<<2))
#define REG_VQMEM_MASK_HIGH (R2_BASE_ADDR + ((OFFSET+0x2A)<<2))
...
#define QM_CTL1_ADDR (QM_BASE_ADDR + 0x10)

#define SAME_PHASE_EN    (1<<16)
...
#define DMA2D_ENABLE() { \
    ...
    \
    regval = REG32(QM_CTL1_ADDR); \
    REG32(QM_CTL1_ADDR) = (regval | SAME_PHASE_EN); \
    regval2 = REG32(QM_CTL1_ADDR); \
    DBG_PRINTF(("DMA2D_ENABLE [0x%x] = (0x%x, 0x%x), [%s, %d]\n", QM_CTL1_ADDR, regval, regval2, __FUNCTION__, __LINE__)); \
...
//N: 8bytes unit for loading source memory
...
#define QM_DEST_OFFSET2 2368//2624//400
