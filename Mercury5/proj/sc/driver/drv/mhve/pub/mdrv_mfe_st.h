...
typedef union mmfe_parm {
    ...
    struct mmfe_parm_res {
        ...
        int                 i_pict_w;       //!< picture width. (un-aligned)
        int                 i_pict_h;       //!< picture height.(un-aligned)
        enum mmfe_pixfmt    i_pixfmt;       //!< pixel format.
        int                 i_outlen;       //!< output length: '<0' mmap-mode, '>=0' user-mode.
        int                 i_inlen;        //!< input  length: '<0' plus-mode, '>=0' user-mode.
        int                 i_flags;        //!< flags.
        char               	i_ClkEn;
        char               	i_ClkSor;
    } res;              //!< used to set resource parameters.
    //! set res parameter share buffer streaming.
    struct mmfe_parm_sharebuf {
        char                i_ShareBufEn;      //!< pixel format.
    } sharebuf;              //!< used to set resource parameters.
...
typedef struct mmfe_buff {
    ...
    struct {
    struct {
    unsigned int    phys;       //!< physical start address.
    unsigned int    phys_ed;    //!< physical end address.
