...
#ifndef __DRV_MACROS_H__
#define __DRV_MACROS_H__
...
#define HalFlashCmdCfiQuery( _ADDR_ ) do{\
        *(volatile u8*)( FLASH_ADDR_KEEPHIGH(_ADDR_) | (0x555L<<1)) = 0x98;\
    }while(0)
...
/********************************************************************************************
 *
 * @brief Toshiba commands
 *
 ********************************************************************************************/

#define TOSHIBA_OFFSET_TO_MANUFACTURE_CODE    (0x00<<1)   ///< Offset to device Manufacture Code
#define TOSHIBA_OFFSET_TO_DEVICE_CODE         (0x01<<1)   ///< Offset to device Device Code
#define TOSHIBA_OFFSET_TO_PROTECTFLAG         (0x02<<1)   ///< Offset to block protection status
...
#define FlashCmdToshibaProtectBlock(_ADDR_) do { \
        HalFlashInitWriteCmdSeq( (_ADDR_) );                        \
        *(volatile u8*)(_ADDR_ | FLASH_ADDOFF_555) = 0x60;    \
        *(volatile u8*)(_ADDR_ | TOSHIBA_OFFSET_TO_PROTECTFLAG) = 0x68;                \
    } while(0)
...
#define FlashCmdToshibaUnProtectBlock(_ADDR_) do{ \
        HalFlashInitWriteCmdSeq( (_ADDR_) );                        \
        *(volatile u8*)( ( FMS_ADDR_KEEPHIGH(_ADDR_) ) | FLASH_ADDOFF_555) = 0x60;    \
        *(volatile u8*)(_ADDR_ | TOSHIBA_OFFSET_TO_PROTECTFLAG) = 0x60;                \
    } while(0)

#define FlashCmdToshibaHiddenRomExit(_ADDR_) do {\
    HalFlashCmdAmdAutoSelect(_ADDR_); \
    *(volatile u8*)((_ADDR_) | FLASH_ADDOFF_555) = 0x00; \
    } while (0)

#define FlashCmdToshibaReadID(value, u32Addr, offset) \
    do { \
        HalFlashCmdAmdAutoSelect((u32Addr)); \
        (value) = *(volatile u16*)((u32Addr) | (offset)); \
        HalFlashCmdAmdReset(u32Addr); \
    } while (0)

#define FlashCmdToshibaetProtectStatus(status, u32Addr) \
    FlashCmdToshibaReadID(status, u32Addr, TOSHIBA_OFFSET_TO_PROTECTFLAG)


#define HalFlashCmdSpansionVSReadStatus(_ADDR_) ( *(volatile u8*)(_ADDR_) )
...
/**
  ...
  * @param _ADDR_ Targeted address
  */
#define HalFlashCmdSpansionVSClearStatus(_ADDR_) do { *(volatile u8*)((_ADDR_)|FLASH_ADDOFF_555) = 0x71; } while(0)

#define HalFlashCmdSpansionVSReset(_ADDR_)       do { *(volatile u8*)(_ADDR_) = 0xF0; } while(0)

#endif //__DRV_MACROS_H__
