...
#define SENSOR_PWDN_POL     CUS_CLK_POL_NEG         // AR0238 no care, OE_BAR pull low always enable
#define SENSOR_RST_POL      CUS_CLK_POL_NEG         // AR0238 RESET active LOW
...
int ar0238_PARL_cus_camsensor_release_handle(ms_cus_sensor *handle);
...
static I2C_ARRAY Sensor_id_table[] =
                {
                 {0x3000, 0x1256},            // ar0238
                //{0x300E, 0x0256},    // ar0238
                };
...
//              {0x3088, 0x8000},            // SEQ_CTRL_PORT
                ...
                //{0x300C, 0x044C/* + 0xF0*/},            // LINE_LENGTH_PCK,    HT,1100
                {0x300C, 0x0444/* + 0xF0*/},            // LINE_LENGTH_PCK,    HT,1100
                ...
                {0x3200, 0x0001},            //BIT0:  AdaCD disable/enable , BIT1: 0=Standa
#if 0 //Test pattern
                {0x3070, 0x0001},
                {0x3072, 0x1234},
                {0x3074, 0x5678},
...
//     SensorReg_Read(usreg, &sen_data);
    //UartSendTrace("ar0238 reg: 0x%x, data: 0x%x, read: 0x%x.\n", usreg, usval, sen_data);
...
static int pCus_I2CRead(ms_cus_sensor *handle, unsigned short usreg, unsigned short* pusval)
{
    ...
    *pusval = usread_data;
    //UartSendTrace("ar0238 reg: 0x%x, data: 0x%x\n", usreg, usread_data);
...
{
    short index = 0;
    ar0238_params *params = (ar0238_params *)handle->private_data;
...
static int AR0238_GetShutterInfo(struct __ms_cus_sensor* handle, CUS_SHUTTER_INFO *info)
{
...
int ar0238_PARL_cus_camsensor_init_handle(ms_cus_sensor* drv_handle)
{
    ...
    SENSOR_DMSG("[%s]", __FUNCTION__);
#if defined(__RTK_OS__)
    sprintf(str, "AR0238linear");
    eCamOsRet = CamOsDirectMemAlloc(str, sizeof(ar0238_params), &pvBufVirt, &u64BufMiu, &u64BufPhy);
    if (eCamOsRet != CAM_OS_OK) {
        UartSendTrace("[%s:%d] fail!\n", __FUNCTION__, __LINE__);
        return FAIL;
    }
    handle->private_data = (ar0238_params *)pvBufVirt;
    memset(handle->private_data, 0, sizeof(ar0238_params));
#else
    handle->private_data = calloc(1, sizeof(ar0238_params));
#endif
    ...
    handle->pCus_sensor_release = ar0238_PARL_cus_camsensor_release_handle;
    ...
    handle->pCus_sensor_GetShutterInfo = AR0238_GetShutterInfo;
#if 1//defined(__MV5_FPGA__)
...
#if defined(__RTK_OS__)
        eCamOsRet = CamOsDirectMemRelease((void*)handle->private_data, sizeof(ar0238_params));
        if (eCamOsRet != CAM_OS_OK) {
...
#if defined(__RTK_OS__)
struct SensorMapTable_t gsensor_map_ar0238 = {
    SENSOR_CHANNEL_NUM,
    ar0238_PARL_cus_camsensor_init_handle,
    ar0238_PARL_cus_camsensor_release_handle
};

int AR0238_Module_Init(void)
{
    cameraSensorRegister(&gsensor_map_ar0238);

    return 0;
}

void AR0238_Module_Exit(void)
{
    cameraSensorUnRegister(&gsensor_map_ar0238);
}

#if defined(__GNUC__)
__section__(initcall6)
#endif
ait_module_init(AR0238_Module_Init);
//ait_module_exit(AR0238_Module_Exit);
#endif //#if defined(__RTK_OS__)

#endif //#if defined(BIND_SENSOR_AR0238) && (BIND_SENSOR_AR0238 == 1)
