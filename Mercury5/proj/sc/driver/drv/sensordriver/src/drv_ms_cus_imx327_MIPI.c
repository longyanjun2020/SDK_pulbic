...
#if (SENSOR0 == IMX327)
#define BIND_SENSOR_IMX327 (1)
#endif
#endif //#if defined(__RTK_OS__)

#if defined(BIND_SENSOR_IMX327) && (BIND_SENSOR_IMX327 == 1)
...
#define vts_30fps_HDR_DOL  1184//1158//1125                             //for 29.091fps @ MCLK=36MHz //VMAX
...
#define Preview_HEIGHT_HDR_DOL_M5U  1080
...
static int pCus_poweron(ms_cus_sensor *handle)
{
    ...
    //sensor_if->SetCSI_VC0HSmode(handle,PACKET_FOOTER_EDGE); //TBD
    //sensor_if->SetCSI_LongPacketType(handle,CSI_LONG_PACKET_TYPE_RAW12|CSI_LONG_PACKET_TYPE_RAW10); //TBD
...
static int pCus_init_mipi4lane_HDR_DOL(ms_cus_sensor *handle)
{
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_HDR_DOL_4lane);i++)
    {
        ...
        else
        {
            ...
            {
                cnt++;
                //printf("Sensor_init_table_HDR_DOL_4lane -> Retry %d...\n",cnt);
                if(cnt>=10)
                {
                    //printf("[%s:%d]Sensor init fail!!\n", __FUNCTION__, __LINE__);
                    return FAIL;
...
#if defined(__RTK_OS__)
    sprintf(str, "IMX327linear");
    eCamOsRet = CamOsDirectMemAlloc(str, sizeof(imx327_params), &pvBufVirt, &u64BufMiu, &u64BufPhy);
...
#else
    handle->private_data = calloc(1, sizeof(imx327_params));
#endif
...
int cus_camsensor_init_handle_hdr_dol_sef1(ms_cus_sensor* drv_handle)
{
    ...
    handle->video_res_supported.res[0].width = Preview_WIDTH;
	if (DrvPM_GetChipID() == CHIP_ID_M5){ //work around part 
		...
		handle->video_res_supported.res[0].height = Preview_HEIGHT_HDR_DOL_M5U;
	}
	handle->video_res_supported.res[0].nOutputWidth = 1948;//0x7A0;//Preview_WIDTH;
	if (DrvPM_GetChipID() == CHIP_ID_M5U){
		handle->video_res_supported.res[0].nOutputHeight = 0x442;//(0x89C >> 1);//0x449;//DOL HDR //Preview_HEIGHT;
	}
	if (DrvPM_GetChipID() == CHIP_ID_M5){
		handle->video_res_supported.res[0].nOutputHeight = (0x97A >> 1) - 1;
	}
#if (SENSOR_MIPI_LANE_NUM == 2)
#endif
#if (SENSOR_MIPI_LANE_NUM == 4)
...
int cus_camsensor_release_handle(ms_cus_sensor *handle)
{
    ...
    //ISensorIfAPI *sensor_if = &handle->sensor_if_api; //TBD
    //sensor_if->PCLK(NULL,CUS_PCLK_OFF); //TBD
    //sensor_if->SetCSI_Clk(handle,CUS_CSI_CLK_DISABLE); //TBD
...
#if defined(__RTK_OS__)
        eCamOsRet = CamOsDirectMemRelease((void*)handle->private_data, sizeof(imx327_params));
        if (eCamOsRet != CAM_OS_OK) {
...
int cus_camsensor_init_handle_hdr_dol_lef(ms_cus_sensor* drv_handle)
{
    ...
    handle->video_res_supported.res[0].nOutputWidth = 1948;//0x7A0;//Preview_WIDTH;
	if (DrvPM_GetChipID() == CHIP_ID_M5U){
		handle->video_res_supported.res[0].nOutputHeight = 0x449;//(0x89C >> 1);//0x449;//DOL HDR //Preview_HEIGHT;
	}
	if (DrvPM_GetChipID() == CHIP_ID_M5){
		 handle->video_res_supported.res[0].nOutputHeight = (0x97A >> 1) - 1;
	}
...
#if defined(__RTK_OS__)
struct SensorMapTable_t gsensor_map_imx327_linear[] = {
    { //Channel 0
        SENSOR_CHANNEL_NUM,
...
struct SensorMapTable_t gsensor_map_imx327_hdr_dol[] = {
    { //Channel 0
        SENSOR_CHANNEL_NUM,
...
int IMX327_Module_Init(void)
{
    cameraSensorRegister(&gsensor_map_imx327_linear[0]);
    cameraSensorRegister(&gsensor_map_imx327_hdr_dol[0]);
    cameraSensorRegister(&gsensor_map_imx327_hdr_dol[1]);

    return 0;
}

void IMX327_Module_Exit(void)
{
    cameraSensorUnRegister(&gsensor_map_imx327_linear[0]);
    cameraSensorUnRegister(&gsensor_map_imx327_hdr_dol[0]);
    cameraSensorUnRegister(&gsensor_map_imx327_hdr_dol[1]);
}

#if defined(__GNUC__)
__section__(initcall6)
#endif
ait_module_init(IMX327_Module_Init);
//ait_module_exit(IMX327_Module_Exit);
#endif //#if defined(__RTK_OS__)

#endif //#if defined(BIND_SENSOR_IMX327) && (BIND_SENSOR_IMX327 == 1)
