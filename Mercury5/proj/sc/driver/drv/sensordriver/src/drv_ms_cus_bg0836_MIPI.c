...
#if (SENSOR0 == BG0836)
#define BIND_SENSOR_BG0836_MIPI (1)
#endif
#endif //#if defined(__RTK_OS__)

#if defined(BIND_SENSOR_BG0836_MIPI) && (BIND_SENSOR_BG0836_MIPI == 1)
...
#define SENSOR_CHANNEL_BG0836_ISP_CALIBRATION_ENABLE (1)
...
#define BG0836A     0x01
#define BG0836C1    0x07
#define BG0836C2    0x0b
#define BG0836B1    0x03
#define BG0836B2    0x0f
...
#define SENSOR_MAX_GAIN      (60 * 1024)                  // max sensor again, a-gain * conversion-gain*d-gain
...
#define Preview_line_period 29630//30556//30580                  //(36M/37.125M)*30fps=29.091fps(34.375msec), hts=34.375/1125=30556,
//#define Line_per_second     32727
#define vts_30fps  1133//1090 8+VSIZE+VBLANK                             //for 29.091fps @ MCLK=36MHz
...
I2C_ARRAY Sensor_init_table_1lane[] =
{
    ...
    {0x0013, 0x0001},
    {0x0021, 0x0000},//0
    {0x0022, 0x0025},//25 30fps  // fe 25fps
    {0x0028, 0x0000},
    {0x0029, 0x0030},
    {0x002a, 0x0000},
    {0x002b, 0x0050},
...
static int pCus_GetSensorID(ms_cus_sensor *handle, u32 *id)
{
    ...
    *id =0;
    ...
    pid_6bit = pid & 0x3f;
    if (pid != 0x4b && pid_6bit != BG0836B1 && pid_6bit != BG0836B2) {
    ...
    SENSOR_DMSG("[%s]BG0836 sensor ,Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);

    return SUCCESS;
}

static int bg0836_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
#if (__SENSOR_W)
...
static int pCus_I2CWrite(ms_cus_sensor *handle, unsigned short usreg, unsigned short usval)
{
    ...
    //UartSendTrace("bg0836_MIPI reg: 0x%x, data: 0x%x, read: 0x%x.\n", usreg, usval, sen_data);
...
static int pCus_I2CRead(ms_cus_sensor *handle, unsigned short usreg, unsigned short* pusval)
{
    ...
    *pusval = usread_data;
    UartSendTrace("bg0836_MIPI reg: 0x%x, data: 0x%x\n", usreg, usread_data);
...
static int pCus_init_mipi1lane_linear(ms_cus_sensor *handle)
{
    ...
    UartSendTrace("BG0836 Sensor_init_table_1lane\n");
...
//debug hxb20180418
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_1lane);i++)
    {
        ...
        if (sen_data != Sensor_init_table_1lane[i].data) {
            UartSendTrace("BG0836 reg write error: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_init_table_1lane[i].reg, Sensor_init_table_1lane[i].data, sen_data);
        }
    }
    for(i=0;i<768;++i)
    {
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"BG0836_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode = bg0836_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
...
static int bg0836_SetPatternMode_isp_calibration( ms_cus_sensor *handle, u32 mode )
{
    return SUCCESS;
}
...
static int BG0836_GetShutterInfo_isp_calibration( struct __ms_cus_sensor* handle, CUS_SHUTTER_INFO *info )
{
...
int cus_camsensor_init_handle_linear_isp_calibration(ms_cus_sensor* drv_handle)
{
    ...
    handle->pCus_sensor_SetPatternMode = bg0836_SetPatternMode_isp_calibration;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = BG0836_GetShutterInfo_isp_calibration;
#if 1//defined(__MV5_FPGA__)
...
#if (SENSOR_CHANNEL_BG0836_ISP_CALIBRATION_ENABLE == 1)
...
{
    cameraSensorRegister(&gsensor_map_bg0836[0]);

#if (SENSOR_CHANNEL_BG0836_ISP_CALIBRATION_ENABLE == 1)
    cameraSensorRegister(&gsensor_map_bg0836[1]);
#endif
    return 0;
}
...
{
    cameraSensorUnRegister(&gsensor_map_bg0836[0]);

#if (SENSOR_CHANNEL_BG0836_ISP_CALIBRATION_ENABLE == 1)
    cameraSensorUnRegister(&gsensor_map_bg0836[1]);
#endif
}
...
#endif //#if defined(BIND_SENSOR_BG0836_MIPI) && (BIND_SENSOR_BG0836_MIPI == 1)
