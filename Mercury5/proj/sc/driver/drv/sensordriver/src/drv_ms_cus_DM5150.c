...
#if defined(BIND_SENSOR_DM5150) && (BIND_SENSOR_DM5150 == 1)
...
#define SENSOR_I2C_ADDR    0xBA                   //I2C slave address
...
int DM5150_cus_camsensor_release_handle(ms_cus_sensor *handle);
...
{
#if 0//defined(DM5150_TEST_PATTERN_EN) && (DM5150_TEST_PATTERN_EN == 1)
    {0x5F, 0x21},
    {0x5D, 0x80},
#else
    {0x0F, 0x02},
...
static int DM5150_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
    SENSOR_DMSG("[%s]\n", __FUNCTION__);
...
static int pCus_I2CWrite(ms_cus_sensor *handle, unsigned short usreg, unsigned short usval)
{
    ...
    UartSendTrace("DM5150 reg: 0x%x, data: 0x%x, read: 0x%x.\n", usreg, usval, sen_data);
...
static int pCus_I2CRead(ms_cus_sensor *handle, unsigned short usreg, unsigned short* pusval)
{
    ...
    *pusval = usread_data;
    UartSendTrace("DM5150 reg: 0x%x, data: 0x%x\n", usreg, usread_data);
...
static int pCus_init(ms_cus_sensor *handle)
{
    ...
    //UartSendTrace("\n================\n");
    //UartSendTrace("DM5150 reg: 0x%x, read: 0x%x.\n",0x7A, sen_data);
...
{
    char index=0;
    DM5150_params *params = (DM5150_params *)handle->private_data;
...
static int DM5150_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int DM5150_cus_camsensor_init_handle(ms_cus_sensor* handle)
{
#if defined(__RTK_OS__)
...
#if defined(__RTK_OS__)
    sprintf(str, "DM5150linear");
    eCamOsRet = CamOsDirectMemAlloc(str, sizeof(DM5150_params), &pvBufVirt, &u64BufMiu, &u64BufPhy);
    if (eCamOsRet != CAM_OS_OK) {
        UartSendTrace("[%s:%d] fail!\n", __FUNCTION__, __LINE__);
        return FAIL;
    }
    handle->private_data = (DM5150_params *)pvBufVirt;
    memset(handle->private_data, 0, sizeof(DM5150_params));
#else
    handle->private_data = calloc(1, sizeof(DM5150_params));
#endif
    DM5150_params *params = (DM5150_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"DM5150_PARL");
    ...
    handle->pCus_sensor_release     = DM5150_cus_camsensor_release_handle;
    ...
    handle->pCus_sensor_SetPatternMode = DM5150_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = DM5150_GetShutterInfo;
...
#if defined(__RTK_OS__)
        eCamOsRet = CamOsDirectMemRelease((void*)handle->private_data, sizeof(DM5150_params));
        if (eCamOsRet != CAM_OS_OK) {
...
#if defined(__RTK_OS__)
struct SensorMapTable_t gsensor_map_DM5150 = {
    SENSOR_CHANNEL_NUM,
    DM5150_cus_camsensor_init_handle,
    DM5150_cus_camsensor_release_handle
};

int DM5150_Module_Init(void)
{
    cameraSensorRegister(&gsensor_map_DM5150);

    return 0;
}

void DM5150_Module_Exit(void)
{
    cameraSensorUnRegister(&gsensor_map_DM5150);
}

#if defined(__GNUC__)
__section__(initcall6)
#endif
ait_module_init(DM5150_Module_Init);
//ait_module_exit(DM5150_Module_Exit);
#endif //#if defined(__RTK_OS__)

#endif //#if defined(BIND_SENSOR_DM5150) && (BIND_SENSOR_DM5150 == 1)
