...
#if (SENSOR0 == RN6752)
#define BIND_SENSOR_RN6752_MIPI (1)
#endif
#endif //#if defined(__RTK_OS__)

#if (defined(BIND_SENSOR_RN6752_MIPI) && (BIND_SENSOR_RN6752_MIPI == 1))
...
#define SENSOR_MODE_FHD_1CH (2)
#define SENSOR_MODE         (SENSOR_MODE_FHD_1CH)
...
#define SENSOR_MODEL_ID     "RN6752_MIPI"
...
int rn6752_MIPI_cus_camsensor_release_handle( ms_cus_sensor *handle );
...
static I2C_ARRAY Sensor_init_table_HD_1CH[] =
{
    ...
    // pin24/23 data lane0
    // pin16/15 clock lane
    // pin18/17 floating, pin14/13 floating, pin12/11 floating
...
static int pCus_I2CWrite(ms_cus_sensor *handle, unsigned short usreg, unsigned short usval)
{
    ...
    UartSendTrace("rn6752_MIPI reg: 0x%x, data: 0x%x, read: 0x%x.\n", usreg, usval, sen_data);
    #endif
...
static int pCus_I2CRead(ms_cus_sensor *handle, unsigned short usreg, unsigned short* pusval)
{
    ...
    *pusval = usread_data;
    UartSendTrace("rn6752_MIPI reg: 0x%x, data: 0x%x\n", usreg, usread_data);
...
int rn6752_MIPI_cus_camsensor_init_handle( ms_cus_sensor* drv_handle )
{
    ...
    if(SENSOR_CHANNEL_MODE_LINEAR == CUS_SENSOR_CHANNEL_MODE_REALTIME_ISP_BYPASS) {
    	handle->interface_attr.attr_mipi.mipi_yuv_order = CUS_SENSOR_YUV_ORDER_CY;
	}
    else {
    	handle->interface_attr.attr_mipi.mipi_yuv_order = CUS_SENSOR_YUV_ORDER_YC;//don't care in RGB pattern. //???
    }
    ...
    handle->pCus_sensor_release = rn6752_MIPI_cus_camsensor_release_handle;
...
struct SensorMapTable_t gsensor_map_rn6752[] = {
    { //Channel 0
        SENSOR_CHANNEL_NUM,
        rn6752_MIPI_cus_camsensor_init_handle,
        rn6752_MIPI_cus_camsensor_release_handle
    }
};

int RN6752_Module_Init(void)
{
    cameraSensorRegister(&gsensor_map_rn6752[0]);
...
{
    cameraSensorUnRegister(&gsensor_map_rn6752[0]);
}
...
#endif //#if defined(BIND_SENSOR_RN6752_MIPI) && (BIND_SENSOR_RN6752_MIPI == 1)
