...
#define usleep(usec)  MsSleep(VM_RTK_MS_TO_TICK(1));
...
#if 0
#define CUS_SEN_INIT_HDL_FUNC          cus_camsensor_init_handle
#define CUS_SEN_RELZ_HDL_FUNC          cus_camsensor_release_handle
#define CUS_SEN_FIF_FUNC               cus_camsensor_fif

#define SUFX(FUNC)              FUNC##_p
#define __STR(FUNC)             #FUNC
#define STR(FUNC)               __STR(FUNC)

#define __SENSOR_PFX(name, function) name##_##function
#define SENSOR_PFX(name, function) __SENSOR_PFX(name, function)
/** @brief Reserved , do not change */
#define CHECK_NULL_ARRAY(ary)  \
{ \
    if(sizeof(ary)/sizeof(ary[0]) == 0) { \
        LOGV("[%s] %s is NULL", __FUNCTION__, __STR(ary)); \
        if (SENSOR_DBG) \
            return FAIL;    \
    }\
}

#define CAP_SET(f) (0x01<<(f))

struct __ms_cus_sensor; /**< Sensor driver handle */
struct __ISensorAPI; /**< Sensor to ISP control interface */
typedef struct __ms_cus_sensor* (*SUFX(CUS_SEN_INIT_HDL_FUNC))(int id); /**< Reserved , do not change */
typedef int (*SUFX(CUS_SEN_RELZ_HDL_FUNC))(struct __ms_cus_sensor*); /**< Reserved , do not change */
typedef void* (*SUFX(CUS_SEN_FIF_FUNC))(struct __ms_cus_sensor*); /**< Reserved , do not change */
#endif
...
typedef enum {
    CUS_FRAME_INACTIVE, /**< Frame end */
    CUS_FRAME_ACTIVE,/**< Frame start */
...
#define CUS_SENSOR_PAD_GROUP_A (0)
#define CUS_SENSOR_PAD_GROUP_B (1)
...
typedef enum
{
    ...
    CUS_SENSOR_CHANNEL_MODE_RAW_STORE_ISP_CALIBRATION = 4,
    CUS_SENSOR_CHANNEL_MODE_RAW_STORE_LI_HDR = 5, //Line Interleaved sequencer HDR mode, Ex: C2390/AR0239
    CUS_SENSOR_CHANNEL_MODE_RAW_STORE_FETCH = 6,
    CUS_SENSOR_CHANNEL_MODE_REALTIME_ISP_BYPASS = 7,
    CUS_SENSOR_CHANNEL_MODE_REALTIME_16B_COMBINE_HDR = 8,
    CUS_SENSOR_CHANNEL_MODE_RAW_STORE_FETCH_16B_COMBINE_HDR = 9,
    CUS_SENSOR_CHANNEL_MODE_REALTIME_14B_COMBINE_HDR = 10, // PS5260 RAW 14 combined HDR
    CUS_SENSOR_CHANNEL_MODE_REALTIME_SEP_LI_HDR = 11, //only M5U support
    CUS_SENSOR_CHANNEL_MODE_RAW_STORE_SEP_LI_HDR = 12, //only M5U support
}CUS_SENSOR_CHANNEL_MODE;

typedef enum
{
	CUS_SENSOR_DCGHDR_NONE = 0,
	CUS_SENSOR_DCGHDR_HALFREALTIME,
	CUS_SENSOR_DCGHDR_DUALRAWSTORE,
	CUS_SENSOR_DCGHDR_DUALREALTIME,
	CUS_SENSOR_DCGHDR_MAX,
}CUS_SENSOR_DCGHDR_MODE;
typedef struct {
    unsigned int gain;
    unsigned int offset;
} CUS_GAIN_GAP_ARRAY;
...
typedef struct {
    ...
    unsigned char Orders[MAX_RUN_ORDER];
    unsigned char CurTaskType;
} CUS_INT_TASK_ORDER;
...
typedef struct {
    ...
    u32 max;       /**< maximun shutter in ns*/
    u32 min;        /**< minimum shutter in ns*/
    u32 step;       /**< shutter in step ns*/
} CUS_SHUTTER_INFO;
...
#if 0
#define CSI_LONG_PACKET_TYPE_NULL  (0x1)                            /**<  Enable  MIPI NULL long packet type*/
#define CSI_LONG_PACKET_TYPE_YUV422_8B  (0x1<<14)          /**<  Enable  MIPI YUV422 long packet type*/
#define CSI_LONG_PACKET_TYPE_RAW8  (0x1<<26)                   /**<  Enable  MIPI RAW8 long packet type*/
#define CSI_LONG_PACKET_TYPE_RAW10  (0x1<<27)                 /**<  Enable  MIPI RAW10 long packet type*/
#define CSI_LONG_PACKET_TYPE_RAW12  (0x1<<28)                 /**<  Enable  MIPI RAW12 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF1  (0x1<<32)          /**<  Enable  MIPI user define 1 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF2  (0x1<<33)          /**<  Enable  MIPI user define 2 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF3  (0x1<<34)          /**<  Enable  MIPI user define 3 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF4  (0x1<<35)          /**<  Enable  MIPI user define 4 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF5  (0x1<<36)          /**<  Enable  MIPI user define 5 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF6  (0x1<<37)          /**<  Enable  MIPI user define 6 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF7  (0x1<<38)          /**<  Enable  MIPI user define 7 long packet type*/
#define CSI_LONG_PACKET_TYPE_USER_DEF8  (0x1<<39)          /**<  Enable  MIPI user define 8 long packet type*/
#endif
...
typedef enum
{
    CUS_SNR_PCLK_SR_PAD,    /**< Pixel clock from parallel sensor intarface */
    CUS_SNR_PCLK_MIPI_TOP,  /**< Pixel clock from MIPI intarface */
    CUS_SNR_PCLK_40MHZ,  /**< Reserved, Do not use*/
    CUS_SNR_PCLK_86MHZ, /**< Reserved, Do not use*/
    CUS_SNR_PCLK_MIPI_4LANE_P, /**< Reserved, Do not use*/
} CUS_SNR_PCLK;
...
typedef enum
{
    ...
    SENSOR_DRIVER_MODE_FULL_HD_30P_RESOLUTION_COMBINED_HDR,
} CUS_SNR_RESOLUTION;

typedef enum
{
    CUS_SNR_ANADEC_SRC_NO_READY = 0,
    CUS_SNR_ANADEC_SRC_PAL,
    CUS_SNR_ANADEC_SRC_NTSC,
    CUS_SNR_ANADEC_SRC_HD,   // Remove Later
    CUS_SNR_ANADEC_SRC_FHD,  // Remove Later
    CUS_SNR_ANADEC_SRC_DISCNT,
    CUS_SNR_ANADEC_SRC_NUM
...
typedef struct __ISensorIfAPI //isp sensor interface API
{
    ...
    */
    int (*PowerOff)( int idx, CUS_CLK_POL pol);
	/** @brief Set sensor sw rst pin.
		@param[in] handle Handle to sensor driver.
		@param[in] channel num
		@param[in] onoff.
		@retval SUCCESS or FAIL if error occurs.
		*/
	int (*SWReset)( int ch, bool bONOFF);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*Reset)( int idx, CUS_CLK_POL pol);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*MCLK)(s32 ssensor_num, bool bONOFF, CUS_MCLK_FREQ eMclkSpeed);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*PCLK)( CUS_SNR_PCLK pclk_source);     //senect sensor
    ...
    */
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*SetIOPad)(u32 ulSnrPadNum, CUS_SENIF_BUS ulSnrType, u32 ulSnrPadCfg);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*SetCSI_Clk)(u16 usmipi_pad, CUS_CSI_CLK eClk, CUS_CLK_POL PclkPol);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*SetCSI_Lane)(u16 usmipi_pad, int num_lan, u8 bon_off);
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*SetCSI_LongPacketType)(u64 ctl_flg);
    ...
    */
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*SetCSI_clk_data_skip)(u8 rx_clk_skip_ns,u8 rx_data_skip_ns);
...
typedef struct __ms_cus_sensor{
    ...
    //u16                         mipi_sampling_delay; /** < MIPI start sampling delay */
 //   u32                         sensor_io_pad;

    cus_camsensor_res_list video_res_supported; /** < Resolution list */
   //sensor calibration
    u32 sat_mingain;    /**< Minimum AE gain */
    u32 dgain_remainder;    /**< After applying sensor Again, rest gain must be dgain_remainder.*/
    ...
    union {
        ...
        struct {
            ...
            int (*pCus_sensor_mipi_get_src_type)(struct __ms_cus_sensor* handle, CUS_SNR_ANADEC_SRC_TYPE *psrc_type);
        } attr_mipi;
        ...
        struct {
            ...
            int (*pCus_sensor_bt656_get_src_type)(struct __ms_cus_sensor* handle, CUS_SNR_ANADEC_SRC_TYPE *psrc_type);
        } attr_bt656;
    } interface_attr;
    ...
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*pCus_sensor_poweron)(struct __ms_cus_sensor* handle);

    /** @brief Sensor power off
    @param[in] handle Handle to sensor driver.
    @retval SUCCESS or FAIL if error occurs.
    */
    int (*pCus_sensor_poweroff)(struct __ms_cus_sensor* handle);
    ...
    */
    int (*pCus_sensor_GetVideoRes)(struct __ms_cus_sensor* handle, cus_camsensor_res **res);
    ...
    */
    ...
    /** @brief Get supported Dain remainder.
    @param[in] handle Handle to sensor driver.
    @param[out] dgain_remainder Receive Dgain after applying sensor Again.
    @retval Return SUCCESS or FAIL if error occurs.
    @remark gain: 1x = 1024
    */
    int (*pCus_sensor_GetDGainRemainder)(struct __ms_cus_sensor* handle, u32 *dgain_remainder);
   ...
   */
   ...
   @retval Return SUCCESS or FAIL if error occurs.
   */
   int (*pCus_sensor_GetCsiAnaDataLaneSel)(u32 *DataLaneSel);
     /** @brief Get resolution list number
   @param[in] handle Handle to sensor driver.
   @param[out] ulres_num resolution list number
   @retval Return SUCCESS or FAIL if error occurs.
   */
   int (*pCus_sensor_GetCsiAnaHsTxPolarity)(u32 *HsTxPolarity);
#if 1//defined(__MV5_FPGA__)
    int (*pCus_sensor_I2CWrite)(struct __ms_cus_sensor* handle, unsigned short usreg, unsigned short usval); //Andy Liu
    int (*pCus_sensor_I2CRead)(struct __ms_cus_sensor* handle, unsigned short usreg, unsigned short* pusval); //Andy Liu
#endif
} ms_cus_sensor;
