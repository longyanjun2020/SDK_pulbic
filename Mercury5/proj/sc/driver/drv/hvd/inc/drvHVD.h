...
#define HVD_INIT_HW_MASK                            BMASK(2:0)          ///< HW Type
    #define HVD_INIT_HW_AVC                             BITS(2:0, 0)    ///< HW deflaut: AVC 0X00
    #define HVD_INIT_HW_AVS                             BITS(2:0, 1)    ///< HW : AVS 0X01
    #define HVD_INIT_HW_RM                              BITS(2:0, 2)    ///< HW: RM 0X10
    #define HVD_INIT_HW_MJPEG                           BITS(2:0, 3)    ///< HW: MJPEG 0X11
    #define HVD_INIT_HW_VP8                             BITS(2:0, 4)    ///< HW: VP8 0X100
    #define HVD_INIT_HW_VP6                             BITS(2:0, 5)    ///< HW: VP6 0x101
    #define HVD_INIT_HW_HEVC                            BITS(2:0, 6)    ///< HW: HEVC 0x110
#define HVD_INIT_MAIN_MASK                          BMASK(4:3)          ///< main type
    #define HVD_INIT_MAIN_FILE_RAW                      BITS(4:3, 0)    ///< main type: default: 0X00
    #define HVD_INIT_MAIN_FILE_TS                       BITS(4:3, 1)    ///< main type: 0X01
    #define HVD_INIT_MAIN_LIVE_STREAM                   BITS(4:3, 2)    ///< main type: 0X10
#define HVD_INIT_INPUT_MASK                         BMASK(5:5)          ///< process path for filling BBU table:  file mode. use drive; TSP: use tsp mode
    #define HVD_INIT_INPUT_TSP                          BITS(5:5, 0)    ///< tsp input( default)
    #define HVD_INIT_INPUT_DRV                          BITS(5:5, 1)    ///< driver input
#define HVD_INIT_START_CODE_MASK                    BMASK(6:6)          ///< AVC FILE MODE ONLY: mkv, mp4 container use.
    #define HVD_INIT_START_CODE_REMAINED                BITS(6:6, 0)    //< start code remained.(Defualt)
    #define HVD_INIT_START_CODE_REMOVED                 BITS(6:6, 1)    ///< start code removed.
#define HVD_INIT_UTOPIA_ENVI                            BIT(7)         ///< check MIU sel and set it
#define HVD_INIT_DBG_FW                                 BIT(8)         ///< check FW is debug version or not
#define HVD_INIT_DUAL_ES_MASK                     BMASK(9:9)          ///< Dual ES buffer iput.
    #define HVD_INIT_DUAL_ES_DISABLE                  BITS(9:9, 0)    ///< Disable Dual ES buffer input.
    #define HVD_INIT_DUAL_ES_ENABLE                   BITS(9:9, 1)    ///< Enable Dual ES buffer input.
...
typedef enum
{
    ...
    E_HVD_ERRCODE_RM_VLC,
    E_HVD_ERRCODE_RM_SIZE_OUT_FB_LAYOUT,
}HVD_ErrorCode;
...
typedef struct
{
    HVD_Codec eCodecType;
    MS_U32 u32DataVAddr;
    MS_U32  u32MemUsageSize;
    MS_U16  u16DataSize;
} HVD_FbMemUsage_Param;

//-------------------------------------------------------------------------------------------------
//  Function and Variable
//-------------------------------------------------------------------------------------------------
DLL_PUBLIC void        MDrv_HVD_PowerCtrl(MS_BOOL bEnable);
DLL_PUBLIC void        MDrv_HVD_SetOSRegBase(MS_U32 u32RegBaseAddr);
DLL_PUBLIC HVD_Result  MDrv_HVD_Init(HVD_MemCfg *pStMemCfg, HVD_Init_Settings *pStInitSettings);
DLL_PUBLIC HVD_Result  MDrv_HVD_Rst(MS_BOOL bErrHandle);
DLL_PUBLIC HVD_Result  MDrv_HVD_Play(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_Exit(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_Pause(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_Flush(MS_BOOL bShowLast);
DLL_PUBLIC HVD_Result  MDrv_HVD_StepDisp(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_StepDecode(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_PushQueue(HVD_Packet_Info *pInfo);
DLL_PUBLIC HVD_Result  MDrv_HVD_PushQueue_Fire(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_DecodeIFrame(MS_PHYADDR u32SrcSt, MS_U32 u32SrcSize);

DLL_PUBLIC HVD_Result  MDrv_HVD_SetDataEnd(MS_BOOL bEnd);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDispErrFrm(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDispRepeatField(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSkipDecMode(HVD_Skip_Decode eDecType);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDispSpeed(HVD_Drv_Disp_Speed eSpeed);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSyncActive(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDropMode(HVD_Drop_Disp eMode, MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_RstPTS(MS_U32 u32PTS);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetFrcMode(HVD_FrmRateConv_Mode eMode);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSyncTolerance(MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSyncVideoDelay( MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSyncFreeRunTH(MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSyncRepeatTH(MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetErrConceal(MS_BOOL u32Arg);
DLL_PUBLIC void        MDrv_HVD_SetDbgLevel(HVD_Uart_Level elevel);
DLL_PUBLIC HVD_Result  MDrv_HVD_SeekToPTS(MS_U32 u32PTS);
DLL_PUBLIC HVD_Result  MDrv_HVD_SkipToPTS(MS_U32 u32PTS);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetFreezeImg(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetBlueScreen(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDispOneField(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetISREvent(MS_U32 u32Event, HVD_InterruptCb fnISRHandler);
DLL_PUBLIC MS_BOOL     MDrv_HVD_SetEnableISR(MS_BOOL bEnable);
DLL_PUBLIC MS_BOOL     MDrv_HVD_SetForceISR(MS_BOOL bEnable);
DLL_PUBLIC MS_BOOL     MDrv_HVD_SetMVOPDone(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetVirtualBox(MS_U16 u16Width, MS_U16 u16Height);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDynScalingParam(void *pStAddr, MS_U32 u32Size);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetAutoRmLstZeroByte(MS_BOOL bOn);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDispInfoTH(HVD_Disp_Info_Threshold *DispInfoTH);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetFastDisplay(MS_BOOL bFastDisplay);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetIgnoreErrRef(MS_BOOL bIgnore);
DLL_PUBLIC HVD_Result  MDrv_HVD_ForceFollowDTVSpec(MS_BOOL bEnable);

DLL_PUBLIC MS_BOOL     MDrv_HVD_IsISROccured(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsDispFinish(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsFrameShowed(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsStepDecodeDone(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_CheckDispInfoRdy(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsDispInfoChg(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsIdle(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsSyncStart(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsSyncReach(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsLowDelay(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsIFrmFound(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_Is1stFrmRdy(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_IsAllBufferEmpty(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_IsAlive(void);

DLL_PUBLIC MS_U32      MDrv_HVD_GetBBUVacancy(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_GetDispInfo(HVD_Disp_Info *pinfo);
DLL_PUBLIC MS_U32      MDrv_HVD_GetPTS(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetNextPTS(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetNextDispQPtr(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDataErrCnt(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDecErrCnt(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetESWritePtr(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetESReadPtr(void);
DLL_PUBLIC MS_BOOL     MDrv_HVD_GetCaps(HVD_Codec u32Type);
DLL_PUBLIC MS_U32      MDrv_HVD_GetErrCode(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetPlayMode(HVD_Get_Mode_Status eMode);
DLL_PUBLIC HVD_Get_Play_State MDrv_HVD_GetPlayState(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDecodeCnt(void);
DLL_PUBLIC MS_U8       MDrv_HVD_GetActiveFormat(void);
DLL_PUBLIC const HVD_DrvInfo *MDrv_HVD_GetInfo(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_GetLibVer(const MSIF_Version **ppVersion);
DLL_PUBLIC MS_BOOL     MDrv_HVD_GetStatus(HVD_DrvStatus *pstatus);
DLL_PUBLIC HVD_Result  MDrv_HVD_GetFrmInfo(HVD_Get_Frm_Info_Type eType, HVD_Frame_Info *pInfo);
DLL_PUBLIC MS_BOOL     MDrv_HVD_GetISRInfo(MS_U32 *eType);
DLL_PUBLIC MS_U32      MDrv_HVD_CalLumaSum(HVD_Get_Frm_Info_Type eType);
DLL_PUBLIC MS_U32      MDrv_HVD_GetUserData_Wptr(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetUserData_Packet(MS_U32 u32Idx, MS_U32 *u32Size);
DLL_PUBLIC HVD_Result  MDrv_HVD_GenPattern(HVD_PatternType eType, MS_U32 u32VAddr, MS_U32 *u32Size);
DLL_PUBLIC MS_U32      MDrv_HVD_GetPatternInfo(HVD_Pattern_Info eType);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDynamicScalingInfo(HVD_DynamicScaling_Info eType);

DLL_PUBLIC MS_U32      MDrv_HVD_GetData(HVD_GData_Type eType);
DLL_PUBLIC MS_U32      MDrv_HVD_GetMem_Dbg(MS_U32 u32Addr);
DLL_PUBLIC void        MDrv_HVD_DbgDumpStatus(HVD_DumpStatus eFlag);
DLL_PUBLIC void        MDrv_HVD_SetMem_Dbg(MS_U32 u32Addr, MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetCmd_Dbg(MS_U32 u32Cmd, MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetSettings_Pro( HVD_SSettings_Type eType, MS_U32 u32Arg);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetBalanceBW(MS_U8 qp_cnt, MS_U8 db_cnt, MS_U8 upper);
DLL_PUBLIC MS_S64      MDrv_HVD_GetPtsStcDiff(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDrvFwVer(void);
DLL_PUBLIC MS_U32      MDrv_HVD_GetFwVer(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetFdMaskDelayCnt(MS_U8 u8DelayCnt);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetOutputFRCMode(MS_U8 u8FrameRate, MS_U8 u8Interlace);
DLL_PUBLIC HVD_Result  MDrv_HVD_DispFrame(MS_U32 u32FrmIdx);
DLL_PUBLIC HVD_Result  MDrv_HVD_FreeFrame(MS_U32 u32FrmIdx);
DLL_PUBLIC HVD_Result  MDrv_HVD_EnableDispQue(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_EnableVSizeAlign(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_Disp_Ignore_Crop(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetFRCDropType(MS_U8 u8DropType);
DLL_PUBLIC MS_U32      MDrv_HVD_GetDrvFwVer(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_GetUserDataInfo(HVD_UserData_Info* pUsrInfo);
DLL_PUBLIC MS_BOOL     MDrv_HVD_GetUsrDataIsAvailable(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_SetDTVUserDataMode(MS_U8 u8UserDataMode);
DLL_PUBLIC HVD_Result  MDrv_HVD_ForceInterlaceMode(MS_U8 u8Mode);
DLL_PUBLIC HVD_Result  MDrv_HVD_ShowDecodeOrder(MS_U8 u8Mode);
DLL_PUBLIC MS_BOOL     MDrv_HVD_GetFrmRateIsSupported(void);
DLL_PUBLIC HVD_Result  MDrv_HVD_GetFrmPackingArrSEI(HVD_FrmPackingSEI *pFrmPacking);
DLL_PUBLIC HVD_Result  MDrv_HVD_Enable_New_Slow_Motion(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result  MDrv_HVD_ParserRstDone(MS_BOOL bEnable);

DLL_PUBLIC HVD_Result MDrv_HVD_CC_Init(void);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_SetCfg(MS_U8 u8Operation, MS_U16 u16BufferSize, MS_U8 u8CC608);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_Set_RB_StartAddr(MS_U32 u32StartPAddress, MS_U8 u8CC608);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_SyncRB_RdAddr2WrAddr(MS_U8 u8CC608);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_Adv_RB_ReadAddr(MS_U32 u32EachPacketSize, MS_U8 u8CC608);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_DisableParsing(MS_U8 u8CC608);
DLL_PUBLIC HVD_Result MDrv_HVD_CC_GetInfo(MS_U32 selector, MS_U8 type, MS_U32 *p1, MS_U32 *p2);
DLL_PUBLIC MS_BOOL    MDrv_HVD_CC_IsHvdRstDone(MS_U8 type);
DLL_PUBLIC MS_U8      MDrv_HVD_CC_GetOverflowStatus(MS_U8 u8CC608);
DLL_PUBLIC MS_U32     MDrv_HVD_CC_Get_RB_WriteAddr(MS_U8 u8CC608);
DLL_PUBLIC MS_U32     MDrv_HVD_CC_Get_RB_ReadAddr(MS_U8 u8CC608);
DLL_PUBLIC MS_BOOL    MDrv_HVD_CC_InfoEnhanceMode(MS_BOOL bEnable);

DLL_PUBLIC HVD_Result MDrv_HVD_SetHVDClockSpeed(HVD_ClockSpeed eClockSpeed);
DLL_PUBLIC HVD_Result MDrv_HVD_SetVPUClockSpeed(HVD_ClockSpeed eClockSpeed);

DLL_PUBLIC HVD_Result MDrv_HVD_PushDispQWithRefNum(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_ShowFirstFrameDirect(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_OnePendingBufferMode(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_FrameRateHandling(MS_BOOL bEnable, MS_U32 u32FrameRate);
DLL_PUBLIC HVD_Result MDrv_HVD_AutoExhaustESMode(MS_U32 u32ESbound);
DLL_PUBLIC HVD_Result MDrv_HVD_SetMinTspDataSize(MS_U32 u32Size);
DLL_PUBLIC HVD_Result MDrv_HVD_AVCSupportRefNumOverMaxDBPSize(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_ErrConcealStartSlice1stMB(MS_U32 u32Param);
DLL_PUBLIC HVD_Result MDrv_HVD_ReturnInvalidAFD(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_CorrectFrameRate(MS_BOOL bEnable, MS_U32 u32FrameRate);
DLL_PUBLIC HVD_Result MDrv_HVD_Set_Err_Tolerance(MS_U32 u32Arg);
DLL_PUBLIC HVD_Result MDrv_HVD_UseCpbRemovalDelay(MS_BOOL bEnable);
DLL_PUBLIC MS_U64 MDrv_HVD_GetU64PTS(void);
DLL_PUBLIC HVD_Result MDrv_HVD_RmForceMcuModeEs(MS_BOOL bEnable);
DLL_PUBLIC HVD_Result MDrv_HVD_PVRTimeShiftSeamlessMode(MS_U8 u8Arg);
DLL_PUBLIC HVD_Result MDrv_HVD_GetFbMemUsageSize(HVD_FbMemUsage_Param *pInfo);
DLL_PUBLIC HVD_Result MDrv_HVD_DisableEsFullStop(MS_BOOL bDisable);
