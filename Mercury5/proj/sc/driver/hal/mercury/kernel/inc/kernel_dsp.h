...
typedef struct W5debug_s
{
  u32           rev0;							//0x00
  u32           u32_dbg2dsp_int;				//0x04
  u32           u32_dbg2mcu_int;				//0x08
  u32           u32_int_clr;					//0x0c
  u32           u32_dsp_dump_ctrl1;			//0x10
  u32           u32_dbg2mcu_err_addr;		//0x14
  u32           u32_dbg2mcu_err_event;		//0x18
  u32           rev1;							//0x1c
  u32           u32_dsp_dump_ctrl2;			//0x20
  u32           u32_dsp_dump_st_addr;		//0x24
  u16           u32_dsp_dump_len;			//0x28
  u16           u32_dsp_dump_tick_L;		//0x2A
  u32           rev2;							//0x2c
  u32           u32_dsp_dump_tick_H;		//0x30
  u32           u32_dsp_dump_rx_win;		//0x34
  u32           rev3[2];						//0x38~0x3c
  u32           u32_rxport_dbg_wd_I;			//0x40
  u32           u32_rxport_dbg_wd_Q;		//0x44
  u32           u32_rxport_dbg_rd_I;			//0x48
  u32           u32_rxport_dbg_rd_Q;			//0x4C
  u32           rev4;							//0x50
  u32           u32_audio_lch_wd;			//0x54
  u32           u32_audio_rch_wd;			//0x58
  u32           u32_audio_lch_rd;				//0x5C
  u32           u32_audio_rch_rd;			//0x60
  u32           rev5;							//0x64
  u32           u32_dsp_cur_pc_L;			//0x68
  u32           u32_dsp_cur_pc_H;			//0x6C
  u32           rev6[4];						//0x70~7C
  u32           u32_dsp_dbg_timer_ctrl;		//0x80
  u32           u32_dsp_dbg_timer_len;		//0x84
  u32           u32_dsp_dbg_tick_val_L;		//0x8C
  u32           u32_dsp_dbg_tick_val_H;		//0x90
  u32           u32_dsp_dbg_rx_timer_len;	//0x94
  u32           u32_dsp_dbg_rx_timer_val;	//0x98
} W5debug_t;

typedef struct W5DpRamSfReg_s
{
 u16    u16_Band;                   // 0xA00
 u16    u16_Arfcn;                  // 0xA02
 u32    u32_IfFre;                  // 0xA04
 u32    u32_FreOffset;              // 0xA06
 s16    s16_FixSynDemCoef[12];      // 0xA08
 s16    s16_FixSynNumCoef[4];       // 0xA20
 u16    u16_reserved[2];            // 0xA28 //####To do  : need modity for 4 byte alig#####
}W5DpRamSfReg_t;

typedef struct W5DpRamIqmeInReg_s
{
 u16    exp_sum_i:8;
 u16    exp_sum_win_i:8;
 u16    exp_sum_win_sq_i:8;
 u16    exp_sum_win_sq_mult_q:8;
 u16    exp_sum_win_sq_q:8;
 u16    exp_sum_win_q:8;
 u16    exp_sum_q:6;
 u16    iqme_done:1;
 u16    reserved_1:9;
...
typedef struct W5DpRamIqmeOutReg_s
{
 u16    alpha;
 u16    belta;
 u16    gain_ratio;
...
typedef struct W5DpRamToneRemoverInAndAgcResultReg_s
{
//ToneRemoverIn
 u16    f1_H;
 u16    f1_L;
 ...
 //AgcResult
 u16    rssi_scaling;
}W5DpRamToneRemoverInAndAgcResultReg_t;
...
//yucnhu: Mapping redefined for B2 ROM codes. (B2 V3)
 ...
 u16      u16_FacchDetectThresSaic; // 0x88E - FACCH detection threshold for SAIC
 u16      au16_Free[182];          // 0x890
 u16      u16_End;                 // 0x9FE
#ifdef __TGGE_RF__
 W5DpRamSfReg_t SfReg_param[2];     // 0xA00 ~ 0xA63
 u32        testpadding;
 u16        au16_SfCoef[112];       // 0xA64 ~ 0xB43
 W5DpRamIqmeInReg_t IqmeInReg;      // 0xB44 ~ 0xB62
 W5DpRamIqmeOutReg_t IqmeOutReg; //0xB64 ~ 0xB6A
 W5DpRamToneRemoverInAndAgcResultReg_t ToneRemoverInAndAgcResult;//0xB6C ~ 0xB76
 //Tone remover result
 u16    DCI_Offset; // 0xB78
 u16    DCQ_Offset; // 0xB7A
 u16    au16_Free2[66];
 //Coefficient of FCCH IIR
 u16    real_A1;    //0xC00
 u16    real_A2;
 u16    imag_A1;
 u16    imag_A2;
...
{
 u16 Table[1280];  //AKO - 20/09/05: change from 512 to 1024 size of DPRAM (512 with FPGA, 1024 on G1)
} W5TabDpRam_t;

///////////////////////////////////////////////////////////////////////////////////////////////
//*** Rfic Rx Width
#define DPRAM_RFIC_RX_WIDTH_10     10
...
#define AMR_RX_RATSCCH                          9 /* It is used by HWL to inform HAC the received block is a RATSCCH rather than a speech. Not a DPRAM field definition. */
...
#define DPRAM_FACCH_DETECT_THRES_SAIC_POS       8
#define DPRAM_FACCH_DETECT_THRES_SAIC_MSK       (0x00FF<<DPRAM_FACCH_DETECT_THRES_SAIC_POS)
...
#define DPRAM_DL_FIXED_FIR_ENABLE_FLAG     0x0080
#define DPRAM_DL_FIXED_FIR_INIT_DONE_FLAG     0x0100
#define DPRAM_DL_TEST_SEQUENCE_ENABLE_FLAG  0x0200
...
#define DPRAM_OFFSET_PROFILE                        11
#define DPRAM_MASK_PROFILE                          (0x3)<<DPRAM_OFFSET_PROFILE
#define DPRAM_OFFSET_DL_TEST_SEQUENCE_TYPE   13
#define DPRAM_MASK_DL_TEST_SEQUENCE_TYPE                         (0x7)<<DPRAM_OFFSET_DL_TEST_SEQUENCE_TYPE
...
/* Dummy paging detection */
#define DetectionDummyPchResult                     u16_IrNbBlockMax
#define DetectionDummyPchType41Thd                  u16_IrNbBlockUsed
#define DetectionDummyPchType42ThdLvl1              u16_IrDspVQ
#define DetectionDummyPchType42CntLvl1              u16_IrDspVR
#define DetectionDummyPchType42ThdLvl2              u16_IrCpuVQ
#define DetectionDummyPchType42CntLvl2              u16_IrCpuVR
#define DetectionDummyPchType42ThdLvl3              u16_EdgeAlgoCtrl
#define DetectionDummyPchType42CntLvl3              au16_InterfererCount[0]


#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_1_THD       (96)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_THD_LVL_1 (46)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_CNT_LVL_1 (20)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_THD_LVL_2 (62)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_CNT_LVL_2 (23)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_THD_LVL_3 (81)
#define DPRAM_DETECTION_DUMMY_PCH_TYPE4_2_CNT_LVL_3 (25)

#define IsDummyPchDetected(rES)                     ((rES) & 0x0001)

/* SAIC */  
#define SoftsumThresSaicOff au16_NbResult0[0]
#define SoftsumThresSaicOn  au16_NbResult0[1]
...
typedef struct W5SReg_s
{
  ...
  u16           u16_FbMode;           // 0x02C
  u16           u16_TxBufCtrlLatched; // 0x02E
  u16           u16_RxConfig;         // 0x030
...
#ifdef __TGGE_RF__
#define SREG_TX_SF_EN_POS            7
#define SREG_TX_SF_EN                (0x0001<<SREG_TX_SF_EN_POS)
#endif
...
#define SREG_AMR_CHANNEL_PROC           (0x0004<<SREG_VOICECODEC_ALGO_POS)
#define SREG_AMR_VOICE                  SREG_AMR_CHANNEL_PROC
...
/* RxConfig */
#define SREG_SAIC_BITMAP_POS            0                                           /* 4bits bitmap. For GSM should be set to 0b0001 for NB0                                           */
#define SREG_SAIC_BITMAP_MSK            (0x000f << SREG_SAIC_BITMAP_POS)
#define SREG_SAIC_RESET_POS             4                                           /* would reset TOI averaging, nothing else I think,                                                */
#define SREG_SAIC_RESET_MSK             (0x0001 << SREG_SAIC_RESET_POS)       
#define SREG_SAIC_TOI_AVG_POS           5                                           /* SAIC TOI averaging function (currently unused I think, maybe only in GPRS)                      */
#define SREG_SAIC_TOI_AVG_MSK           (0x0001 << SREG_SAIC_TOI_AVG_POS)     
#define SREG_SAIC_BITMAP_RESET_POS      6                                           /* DSP will latch the SAIC_BITMAP internally when this is set, should be at every framei n general */
#define SREG_SAIC_BITMAP_RESET_MSK      (0x0001 << SREG_SAIC_BITMAP_RESET_POS)
#define SREG_SAIC_NB_POS                7                                           /* enable SAIC on NB according to SAIC_BITMAP bitmap (4bits, LSB is NB0, etc)                      */
#define SREG_SAIC_NB_MSK                (0x0001 << SREG_SAIC_NB_POS)          
#define SREG_SAIC_SB_POS                8                                           /* enable SAIC for SCH                                                                             */
#define SREG_SAIC_SB_MSK                (0x0001 << SREG_SAIC_SB_POS)          
#define SREG_SAIC_ALL_FEATURES_MSK      (SREG_SAIC_BITMAP_MSK | SREG_SAIC_RESET_MSK | SREG_SAIC_TOI_AVG_MSK | SREG_SAIC_BITMAP_RESET_MSK | SREG_SAIC_NB_MSK | SREG_SAIC_SB_MSK)
#define SREG_RX_IQ_15BITS_POS           9                                           /* to be set along with the RxFIR output scaling change, mandatory only if SAIC will be used       */
#define SREG_RX_IQ_15BITS_MSK           (0x0001 << SREG_RX_IQ_15BITS_POS)
#define SREG_NBCCH2FACCH_POS            10
#define SREG_NBCCH2FACCH_ENABLE         (0x0001 << SREG_NBCCH2FACCH_POS)

/* RxConfig: Dummy PCH detection */
#define SREG_DUMMY_PCH_POS              15
#define SREG_DUMMY_PCH_ENABLE           (0x0001 << SREG_DUMMY_PCH_POS)

/* Burst IQ dump */
#define SREG_BURST_IQ_DUMP_POS          14
#define SREG_BURST_IQ_DUMP_ENABLE       (0x0001 << SREG_BURST_IQ_DUMP_POS)
#define SREG_BURST_IQ_FCH_DUMP_POS      15
#define SREG_BURST_IQ_FCH_DUMP_ENABLE   (0x0001 << SREG_BURST_IQ_FCH_DUMP_POS)
...
//** SWDER5 ************
#define SREG_FCH_NCELL                  (0x0001<<0)
#define SREG_FCH_PREFILTER_SHIFT_POS    1
#define SREG_FCH_PREFILTER_SHIFT_MSK    (0x7fff<<SREG_FCH_PREFILTER_SHIFT_POS)

#define SREG_FCH_PREFILTER_SHIFT        2
...
//==================================================
#define SREG_PATCH(iD, rEG, mODE, rEV)  (((rEV) >= (u32)CHIP_ID(BLOSSOM, U01)) ? (SREG_PATCH##iD##_##rEG##_##mODE) : \
                                                      (SREG_PATCH##iD##_##rEG##_##mODE))
/* Patch configuration in idle */
#define SREG_PATCH1_EN_IDLE           0                
#define SREG_PATCH1_BREAK_ADD_IDLE    ((u32)0x00000000)
#define SREG_PATCH1_INST1_IDLE        ((u16)0x0000)    
#define SREG_PATCH1_INST2_IDLE        ((u16)0x0000)    
#define SREG_PATCH2_EN_IDLE           0                
#define SREG_PATCH2_BREAK_ADD_IDLE    ((u32)0x00000000)
#define SREG_PATCH2_INST1_IDLE        ((u16)0x0000)    
#define SREG_PATCH2_INST2_IDLE        ((u16)0x0000)    
#define SREG_PATCH3_EN_IDLE           0                
#define SREG_PATCH3_BREAK_ADD_IDLE    ((u32)0x00000000)
#define SREG_PATCH3_INST1_IDLE        ((u16)0x0000)    
#define SREG_PATCH3_INST2_IDLE        ((u16)0x0000)    
#define SREG_PATCH4_EN_IDLE           0                
#define SREG_PATCH4_BREAK_ADD_IDLE    ((u32)0x00000000)
#define SREG_PATCH4_INST1_IDLE        ((u16)0x0000)    
#define SREG_PATCH4_INST2_IDLE        ((u16)0x0000)    

/* Patch configuration in dedicated */
#define SREG_PATCH1_EN_DEDI           0                
#define SREG_PATCH1_BREAK_ADD_DEDI    ((u32)0x00000000)
#define SREG_PATCH1_INST1_DEDI        ((u16)0x0000)    
#define SREG_PATCH1_INST2_DEDI        ((u16)0x0000)    
#define SREG_PATCH2_EN_DEDI           0                
#define SREG_PATCH2_BREAK_ADD_DEDI    ((u32)0x00000000)
#define SREG_PATCH2_INST1_DEDI        ((u16)0x0000)    
#define SREG_PATCH2_INST2_DEDI        ((u16)0x0000)    
#define SREG_PATCH3_EN_DEDI           0                
#define SREG_PATCH3_BREAK_ADD_DEDI    ((u32)0x00000000)
#define SREG_PATCH3_INST1_DEDI        ((u16)0x0000)    
#define SREG_PATCH3_INST2_DEDI        ((u16)0x0000)    
#define SREG_PATCH4_EN_DEDI           0                
#define SREG_PATCH4_BREAK_ADD_DEDI    ((u32)0x00000000)
#define SREG_PATCH4_INST1_DEDI        ((u16)0x0000)    
#define SREG_PATCH4_INST2_DEDI        ((u16)0x0000)    

/* Patch configuration in packet transfer */
#define SREG_PATCH1_EN_PTM            0                
#define SREG_PATCH1_BREAK_ADD_PTM     ((u32)0x00000000)
#define SREG_PATCH1_INST1_PTM         ((u16)0x0000)    
#define SREG_PATCH1_INST2_PTM         ((u16)0x0000)    
#define SREG_PATCH2_EN_PTM            0                
#define SREG_PATCH2_BREAK_ADD_PTM     ((u32)0x00000000)
#define SREG_PATCH2_INST1_PTM         ((u16)0x0000)    
#define SREG_PATCH2_INST2_PTM         ((u16)0x0000)    
#define SREG_PATCH3_EN_PTM            0                
#define SREG_PATCH3_BREAK_ADD_PTM     ((u32)0x00000000)
#define SREG_PATCH3_INST1_PTM         ((u16)0x0000)    
#define SREG_PATCH3_INST2_PTM         ((u16)0x0000)    
#define SREG_PATCH4_EN_PTM            0                
#define SREG_PATCH4_BREAK_ADD_PTM     ((u32)0x00000000)
#define SREG_PATCH4_INST1_PTM         ((u16)0x0000)    
#define SREG_PATCH4_INST2_PTM         ((u16)0x0000)    
...
/* End of 20101110 */     
#define SREG_TASK1_DC_JUMP_DETECTED               (0x0001<<14)
#define SREG_TASK1_DC_JUMP_CANCELLATION_ENABLE    (0x0001<<15)
...
/************************************************/
extern volatile W5debug_t * const g_ptW5debug;
extern volatile W5Dpram_t * const g_ptW5Dpram;
extern volatile W5TabDpRam_t * const g_ptW5TabDpRam;
extern volatile W5SReg_t *  const g_ptW5SReg;
extern volatile W5DspIrqControl_t *  const g_ptW5DspIrqControl;
extern volatile W5CntReg_t *  const g_ptW5CntReg;
extern volatile W5Dsma_t *  const g_ptW5Dsma;

#endif /*__KERNEL_DSP_H__*/
