...
typedef enum
{
    ...
    RIUBASE_ISP_10 = 0x130C,
    RIUBASE_ISP_11 = 0x130D,
    RIUBASE_ISP_12 = 0x130E,
	RIUBASE_ISP_DMA0 = 0X130F,
	RIUBASE_ISP_DMA2 = 0X1310,
    RIUBASE_MLOAD = 0x1311,
    RIUBASE_SCL_MLOAD = 0x122E,
#if 0
    RIUBASE_VIF = 0x1312,
...
typedef enum
{
    AE_SOURCE_FROM_WB_PRE_GAIN = 0,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_LS = 1,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_CS = 2,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_LS_CS  = 3,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_POST_WB = 4,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_LS_POST_WB = 5,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_CS_POST_WB = 6,
    AE_SOURCE_FROM_WB_PRE_GAIN_MULTIPLY_LS_CS_POST_WB = 7,
    AE_SOURCE_FROM_CS = 8,
    AE_SOURCE_FROM_POST_WB = 12,
} ISP_AE_SOURCE_SELECTION;
...
typedef enum
{
	SEN_YC_FORMAT_SEPARATE_YC,
	SEN_YC_FORMAT_YC16BIT
} SEN_YC_FORMAT; //Sensor input format
...
typedef struct
{
    ISP_CCM_IP  eID;
    MS_BOOL bEnable;
    MS_BOOL  bDitherEnable;
    s16   i2CCM[9];

    MS_BOOL by_sub_16_en;
    MS_BOOL br_sub_16_en;
    MS_BOOL bb_sub_16_en;
    MS_BOOL by_add_16_post_en;
    MS_BOOL br_add_16_post_en;
    MS_BOOL bb_add_16_post_en;
    MS_BOOL bcb_add_128_post_en;
    MS_BOOL bcr_add_128_post_en;
    u32 urran;
    u32 ugran;
    u32 ubran;
} ISP_CCM_CFG;
...
int SCL_MLoadApply(ISP_HAL_HANDLE hnd, MLOAD_IQ_MEM *mload_mem_cfg, MS_BOOL bTrigger);
...
int ISP_SetRDMAAutoTrigSel(ISP_HAL_HANDLE hnd, ISP_PIPE_e isp_pipe, int sel);
int ISP_SenYCFormat(ISP_HAL_HANDLE hnd, ISP_PIPE_e isp_pipe, SEN_YC_FORMAT format);
...
int ISP_AE_SourceSelection(ISP_HAL_HANDLE hnd, volatile ISP_AE_SOURCE_SELECTION eSource);
int ISP_AE_SetWin(ISP_HAL_HANDLE hnd, ISP_AE_WIN win);
...
//Test function
int ISP_PatternMode(ISP_HAL_HANDLE hnd, int width, int height);
//WDMA function
int ISP_WdmaEnable(ISP_HAL_HANDLE hnd,IspDmaId_e id, MS_BOOL enable);
...
int ISP_EnableISP_HDR16to12(ISP_HAL_HANDLE hnd, MS_BOOL enable);
int ISP_EnableISP_Decomp(ISP_HAL_HANDLE hnd, MS_BOOL enable);

#endif //end of HAL_ISP_H
