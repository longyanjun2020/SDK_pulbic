...
typedef enum _SEND_HEADER_STATE
{
    HEADER_SEND_BEGIN = 0,
    ...
    HEADER_URI_SENDING,
    HEADER_URI_SENT,
    HEADER_VERSION_SENDING,
    HEADER_VERSION_SENT,
    HEADER_REQUEST_SENDING,
    HEADER_REQUEST_SENT,
    HEADER_TERMINATE_SENDING,
    HEADER_TERMINATE_SENT,
    
    HEADER_SEND_END
        
}SEND_HEADER_STATE;

typedef enum _HTTP_RECV_STATE
{
    HTTP_RECV_START,

    HTTP_HEADER_RECEIVING,
    HTTP_HEADER_RECEIVED,
    HTTP_CONTENT_RECEIVING,
    HTTP_CONTENT_RECEIVED,
    
    HTTP_RECV_END    
}HTTP_RECV_STATE;

typedef enum _HTTP_CHUNK_STATE
{
    HTTP_CHUNK_START,
    ...
    {
        ven_socket_t        HttpSocket;         // The underling socket
    ...
    UINT32                  HTTPClientGetInHeaderBuffer     (HTTP_SESSION_HANDLE pSession, CHAR* DestBuffer, UINT32 MaxLen, UINT32* Len);                 
    ...
    void                        HTTPClientSetAppHandle        (HTTP_SESSION_HANDLE pSession, UINT32 handle);


    typedef void (*HTTPClientSendRequestCb)(HTTP_SESSION_HANDLE pSession, UINT32 nRet);
    typedef void (*HTTPClientWriteDataCb)(HTTP_SESSION_HANDLE pSession, UINT32 nRet, UINT32 nBytesSent);
    typedef void (*HTTPClientRecvResponseCb)(HTTP_SESSION_HANDLE pSession, UINT32 nRet);
    typedef void (*HTTPClientReadDataCb)(HTTP_SESSION_HANDLE pSession, UINT32 nRet, VOID *pBuffer, UINT32 nBytesRecived);

    typedef struct _HTTP_SEND_REQUEST_PARAM
    {
        HTTP_SESSION_HANDLE pSession;
        HTTPClientSendRequestCb sendRequestCallback;
        HTTPClientWriteDataCb writeDataCallback;
        HTTPClientRecvResponseCb recvResponseCallback;
        HTTPClientReadDataCb readDataCallback;
        BOOL TotalLength;
        char* pUrl;
        void* pData;
        UINT32 nDataLength;
        UINT32 nBytesToRead;
        UINT32 nSendRequestTimeout;
        UINT32 nRecvResponseTimeout;
        UINT32 nReadDataTimeout;
        UINT32 nWriteDataTimeout;
        UINT32 nClientPort;
        
    }HTTP_SEND_REQUEST_PARAM;

    UINT32 HTTPClientSendRequestAsync (HTTP_SEND_REQUEST_PARAM* send_request_param);
    UINT32 HTTPClientWriteDataAsync (HTTP_SESSION_HANDLE pSession, VOID *pBuffer, UINT32 nBufferLength, UINT32 nTimeout, HTTPClientWriteDataCb cb);
    ...
    UINT32                  HTTPIntrnConnectionOpen       (P_HTTP_SESSION pHTTPSession, ven_socket_connect_cbfunc_t callback);
    UINT32                  HTTPIntrnSend                 (P_HTTP_SESSION pHTTPSession, CHAR *pData,UINT32 *nLength, ven_socket_send_cbfunc_t callback);
    ...
    UINT32                  HTTPIntrnHeadersSend          (P_HTTP_SESSION pHTTPSession, HTTP_VERB HttpVerb, ven_socket_send_cbfunc_t callback);
