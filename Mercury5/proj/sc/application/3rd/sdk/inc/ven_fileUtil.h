...
typedef void* ven_fileUtil_search_handle_t;

// File search structure
typedef struct ven_fileUtil_search_start_t_
{
	u32                dwNum;     /* total files(folders) */
	u32                dwCurr;    /* current item*/
	ven_dir_handle_t   hDir;      /* folder handle */
    u16*        pFileNameWC;      /* WildCard string for match*/
}ven_fileUtil_search_start_t;
 ...
 *
 */
s32 ven_fileUtil_findnext(ven_fileUtil_search_handle_t hSearch, ven_file_info_t *pFileInfo);
 ...
 */
ven_fileUtil_search_handle_t ven_fileUtil_findfirst(const u16* pFilePath, ven_file_info_t *pFileInfo);
 ...
 *
 */
s32 ven_fileUtil_findclose(ven_fileUtil_search_handle_t hSearch); 
#endif
