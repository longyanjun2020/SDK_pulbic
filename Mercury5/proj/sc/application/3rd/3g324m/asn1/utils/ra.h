...
typedef struct
{
...
#if defined(RV_RA_DEBUG_MALLOC)
    void        **allocs; /* Array of memory allocations for all elements */
#endif
    ...
    /* Thread safety */
    RvBool      threadSafe; /* RV_TRUE if raAdd and raDelete are thread-safe */
    RvBool      userThreadSafe; /* RV_TRUE if the user is going to do thread-safety with our lock */
