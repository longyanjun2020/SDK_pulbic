...
#ifdef __RTK_OS__
	//-------------------------------------------------------------------
	// sort of POSIX things here
	//-------------------------------------------------------------------
	static UPNP_INLINE char* osal_strdup( const char *s ) {
		char *buf;
		int len = strlen(s);
		buf = osal_malloc(len + 1);
		strcpy( buf, s );
		return buf;
	}

 	static UPNP_INLINE int osal_strerror_r(int errnum, char *buf, size_t buflen) {
		return snprintf( buf, buflen, "ERR: %d", errnum );
	}
#else
	//-------------------------------------------------------------------
	// sort of POSIX things here
	//-------------------------------------------------------------------
 	static UPNP_INLINE char* strdup( const char *s ) {
		char *buf;
		int len = strlen(s);
		buf = osal_malloc(len + 1);
		strcpy( buf, s );
 	...
 	static UPNP_INLINE int strerror_r(int errnum, char *buf, size_t buflen) {
		return snprintf( buf, buflen, "ERR: %d", errnum );
	}

	#define osal_strdup(s) 				strdup(s)
	#define osal_strerror_r(en,b,l) 	strerror_r(en,b,l)
#endif
