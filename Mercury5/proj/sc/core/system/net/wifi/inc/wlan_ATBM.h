...
enum {
    ...
    WLAN_ENCRYPT_WEP_SHARED = 0x10,   /* WEP shard encryption. */
...
typedef enum _WLAN_WPS_STATE
{
    ...
    WLAN_WPS_STATE_FAIL,
    WLAN_WPS_STATE_CONFIGURED,
    WLAN_WPS_STATE_LINK_UP,
    WLAN_WPS_STATE_SEND_EAPOL_START,
    WLAN_WPS_STATE_WAIT_EAPOL_START,
    WLAN_WPS_STATE_WAIT_UPNP_START,
    WLAN_WPS_STATE_WAIT_REQ_ID,
    WLAN_WPS_STATE_WAIT_RESP_ID,
    WLAN_WPS_STATE_WAIT_START,
    WLAN_WPS_STATE_WAIT_M1,
    WLAN_WPS_STATE_SENT_M1,
    WLAN_WPS_STATE_SENT_M2D,
    WLAN_WPS_STATE_WAIT_M2,
    WLAN_WPS_STATE_RX_M2D,
    WLAN_WPS_STATE_WAIT_PIN,
    WLAN_WPS_STATE_WAIT_M3,
    WLAN_WPS_STATE_WAIT_M4,
    WLAN_WPS_STATE_WAIT_M5,
    WLAN_WPS_STATE_WAIT_M6,
    WLAN_WPS_STATE_WAIT_M7,
    WLAN_WPS_STATE_WAIT_M8,
    WLAN_WPS_STATE_WAIT_DONE,
    WLAN_WPS_STATE_WAIT_ACK,
...
int8* WLAN_SYS_GetVersion(void);        //Get  drvier version.
nhw_drv_hdl WLAN_SYS_Start(void);         //set as AP mode or sta mode, if async, should wait wifi start done.
void WLAN_SYS_Stop(nhw_drv_hdl hdl);     //when used in network shutdown, will  call this function after netif shutdown.
int32 WLAN_SYS_GetMode(void);          //get ap or sta mode
void WLAN_SYS_GetMacAddr(uint8* addr);
void WLAN_SYS_GetMacAddrStr(int8* addrStr);
struct pbuf;
void WLAN_SYS_TxPkt(struct pbuf *p);                          //Directly Tx data called by low_level_output
void WLAN_SYS_RxPkt(struct netif *netif, int total_len);   //not used here
int32 WLAN_SYS_GetTXQueueAvailCount(void); //how to use it
void WLAN_SYS_GetStatsInfo(WLAN_STATISTICS_INFO *stats, char *sdio_stat_buf, int sdio_stat_len);   //get static info, RK not set
void WLAN_SYS_InitHw(void);                                  //HW init
void WLAN_SYS_ResetHw(void);                               //RK not set. BCM use GPO reset
bool WLAN_SYS_IsStart(void);                                  //same as BCM and RK
/**************** STA ********************/
int32 WLAN_STA_AllocateScanBuf(void);                     //same as BCM and RK
void WLAN_STA_FreeScanBuf(void);                          //same as BCM and RK
int8* WLAN_STA_ScanBufAddr(void);                        //same as BCM and RK
int8* WLAN_STA_ScanAPbyModule(uint32* uiCount);         //same as BCM and RK
int8* WLAN_STA_GetScanResultByIndex(uint32 uiIndex, int8* pchBuf);   //same as BCM and RK
int32 WLAN_STA_ScanAP(int8* scan_buf, uint32 buf_size);                 //call scan function.
int32 WLAN_STA_PutScanInfo(int8* scan_buf, uint32 buf_size);  //same as BCM and RK
int32 WLAN_STA_OpenScanInfo(uint32* puiFileHandler);         //same as BCM and RK
void WLAN_STA_CloseScanInfo(void);                            //same as BCM and RK
int32 WLAN_STA_ScanAPbySSID(int8* pchSSID, WLAN_BSS_INFO *psInfo); //same as BCM and RK, first scan, then compare SSID
void WLAN_STA_ScanChannel(uint32* puiChannelList);                  //same as BCM and RK
void WLAN_STA_AutoSelectChannel(uint32* puiChannel);                //not define   ???
void WLAN_STA_AutoSelectChannel_PanaRule(uint32* puiChannel);    //not define  ???
int32 WLAN_STA_SetTargetAP(int8 *ssid, WLAN_AUTH_MODE authMode, WLAN_ENCRYPTION encryption, const int8 *key); //same as BCM and RK
int32 WLAN_STA_ConnectTargetAP(void);                              //same as  RK
int32 WLAN_STA_JoinAP(int8 *ssid, WLAN_AUTH_MODE authMode, WLAN_ENCRYPTION encryption, const int8 *key );  //final join AP function.
int32 WLAN_STA_GetConnectedINFO(WLAN_CONNECTION_INFO* info);                                            //get connected AP info..
...
int32 WLAN_AP_Create(int8* ssid,
                     ...
                     bool bHideSSID);                               //create SW ap network
int32 WLAN_AP_GetAssocList(int8* pchBuf, uint32 uiBufSize);  //get sta list,, how many sta required???
int WiFiAPConnectionLimit(void);                               //not defined  ???
