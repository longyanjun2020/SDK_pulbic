/// @ait_only
/** @file mmpd_mkvparser.c
@brief MKV Parser Control driver and host side parser
@author Truman Yang
@author Champ Yen,
...
//extern function
extern MMP_VID_FILEVALIDATION MMPF_MKVPSR_Parse(void);
extern MMP_ERR MMPF_MKVPSR_Demux(void);
...
*/
MMP_ERR MMPD_MKVPSR_Parse(const MMP_ULONG file_size,MMP_VID_FILEVALIDATION *fileValidation,MMP_BOOL bCardMode)
{
	MMP_ERR	mmpstatus;
    ...
    if (bCardMode) {
    	...
    	*fileValidation = MMPF_MKVPSR_Parse();
...
*/
MMP_ERR MMPD_MKVPSR_SeekByTime(const MMP_ULONG ulTime,MMP_BOOL bCardMode, MMP_ULONG ulSeekOption)
{
	MMP_ERR	mmpstatus;
    ...
    if (bCardMode) {
        ...
        return MMP_ERR_NONE;
        #else
        MMP_BOOL bSeekToPrecisePos = ulSeekOption & MMPD_VIDPSR_SEEK_PRECISE;
    	return MMPD_MKVPSR_SeekByTime_Impl(ulTime, bSeekToPrecisePos);
    	#endif
    }
}
...
@return The status of the function call.
*/
MMP_ERR MMPD_MKVPSR_FeedData(MMP_BOOL *pbFinished)
{
