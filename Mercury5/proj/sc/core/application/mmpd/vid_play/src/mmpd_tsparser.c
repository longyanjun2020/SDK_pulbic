/// @sigmastar_only
/** @file mmpd_tsparser.c
@brief TS Parser Control driver and host side parser
@author Tunglin Hsieh
@version 1.0 Original version
*/

/** @addtogroup MMPD_TSPSR
 *  @{
 */
#include "mmp_lib.h"
...
//extern function
extern MMP_VID_FILEVALIDATION MMPF_TSPSR_Parse(void);
extern MMP_ERR MMPF_TSPSR_Demux(void);
...
*/
MMP_ERR MMPD_TSPSR_Parse(const MMP_ULONG file_size,MMP_VID_FILEVALIDATION *fileValidation,MMP_BOOL bCardMode)
{
	MMP_ERR	mmpstatus;
    ...
    if (bCardMode) {
    	...
    	*fileValidation = MMPF_TSPSR_Parse();
...
*/
MMP_ERR MMPD_TSPSR_SeekByTime(const MMP_ULONG ulTime,MMP_BOOL bCardMode, MMP_ULONG ulSeekOption)
{
	MMP_ERR	mmpstatus;
...
@return The status of the function call.
*/
MMP_ERR MMPD_TSPSR_FeedData(MMP_BOOL *pbFinished)
{
