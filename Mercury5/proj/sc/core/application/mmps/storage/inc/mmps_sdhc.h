/// @ait_only
#ifndef _MMPS_SDHC_H_
#define _MMPS_SDHC_H_
...
/**
 ...
 <b>Control Flow:</b><br>
 <pre>
 Step 1: Initial SD interface by calling MMPS_SDHC_InitialInterface().
 ...
 Step 4: Access SD card
   a. Check card in status by calling MMPS_SDHC_CheckCardIn().
   b. Check card write protect status by calling MMPS_SDHC_CheckCardWP().
   c. Check card status by calling MMPF_SDHC_CheckCardStatus().
   d. Get card size by calling MMPS_SDHC_GetSize(), if needed. Returns in Bytes.
   e. Read sector by calling MMPS_SDHC_Read(ulStartSector, *pBuf, ulSectorCnt).
   f. Read sector by calling MMPS_SDHC_Write(ulStartSector, *pBuf, ulSectorCnt).
