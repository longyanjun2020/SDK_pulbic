...
#ifndef __AACPLUS_BASICOP_ARM_H
#define __AACPLUS_BASICOP_ARM_H
 ...
 * saturated to +0x7FFFFFFF.)
 */
__inline Word32 L_mult_arm1(Word16 x, Word32 y)
{
#ifdef	__GNUC__
	Word32 product;
	asm ("smulbt %0, %1, %2" : "=r" (product) : "r" (x) , "r" (y));
	asm ("qadd %0, %0, %0" : "+r" (product));
	return product;
#else	
    Word32 product;
...
__inline Word32 ffr_norm32_lpp_arm(Word32 x)
{
	int lz;
    x ^= (x >> 31);                    /* invert x if it's negative */
#ifdef	__GNUC__
