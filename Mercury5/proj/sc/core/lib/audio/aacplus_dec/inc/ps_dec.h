...
typedef struct {
  ...
  Word16 *aIidPrevFrameIndex;          /*!< IID index of the previous frame */
  Word16 *aIccPrevFrameIndex;          /*!< ICC index of the previous frame */
  ...
  Word16 *aEnvStartStop;              /*!< Start and stop position in time for each envelope */

  Flag *abIidDtFlag;                    /*!< Flags deltacoding in time/freq flag for IID, 0 => freq */
  Flag *abIccDtFlag;                    /*!< Flags deltacoding in time/freq flag for ICC, 0 => freq */

  Word16 *aaIidIndex[MAX_NO_PS_ENV+2]; /*!< The IID index for all envelopes and all IID bins */
  Word16 *aaIccIndex[MAX_NO_PS_ENV+2]; /*!< The ICC index for all envelopes and all ICC bins */
...
#else
//	Word32 H11[NO_IID_GROUPS];
//  Word32 H21[NO_IID_GROUPS];
//  
//  Word32 deltaH11[NO_IID_GROUPS];
//  Word32 deltaH21[NO_IID_GROUPS];
	Word32 H11[NO_IID_GROUPS];
  Word32 H12[NO_IID_GROUPS];
  Word32 H21[NO_IID_GROUPS];
  Word32 H22[NO_IID_GROUPS];

  Word32 deltaH11[NO_IID_GROUPS];
  Word32 deltaH12[NO_IID_GROUPS];
  Word32 deltaH21[NO_IID_GROUPS];
  Word32 deltaH22[NO_IID_GROUPS];
#endif  
}
PS_DEC;
...
extern void deCorrelate_fun1(Word32 *aLeftReal, Word32 *aLeftImag, Word32 aPower[], const Word16 groupBorders[]);            
extern void deCorrelate_fun2(Word32 *aLeftReal, Word32 *aLeftImag, Word32 aPower[], const Word16 groupBorders[], Word16 usb, const Word16 groupShift[]);                                    
extern void appRotate_sub1(Word32 *ptrH, 
                           Word32 *pLeftReal,
                           Word32 *pLeftImag,
                           Word32 *pRightReal,
                           Word32 *pRightImag);
                           
extern void appRotate_sub2(Word32 *ptrH, 
                           Word32 *pLeftReal,
                           Word32 *pLeftImag,
                           Word32 *pRightReal,
                           Word32 *pRightImag,
                           Word16  usb,
                           const Word16 *groupBorders);                                
#endif
