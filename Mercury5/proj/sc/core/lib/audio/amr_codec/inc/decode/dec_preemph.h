...
********************************************************************************
*/
#ifndef dec_preemph_h
#define dec_preemph_h "$Id $"
...
int Decoder_preemphasis_init (preemphasisState *st);
...
int Decoder_preemphasis_reset (preemphasisState *st);
/* reset of preemphasis filter (i.e. set state memory to zero)
   returns 0 on success
 */
void Decoder_preemphasis_exit (preemphasisState *st);
/* de-initialize preemphasis filter (i.e. free status struct)
   stores NULL in *st
 */
 
int Decoder_preemphasis (
