...
#define SD_FW_DUMP_BIN_NORFLASH		(1)	//SD_FW_DUMP_BIN_NAME
...
/** @name WIFI
@{ */
    #define SUPPORT_H264_WIFI_STREAM    (1) // 1: support H264 wifi streaming //TBD
    #define SUPPORT_MJPEG_WIFI_STREAM   (1) // 1: support MJPEG wifi streaming //TBD
...
@{ */
    ...
    /* Enable to run IQ server for tuning with IQ tool */
...
@{ */
    ...
    #define MTS_MUXER_EN                (1)     //REF::#define VR_VIDEO_TYPE               COMMON_VR_VIDEO_TYPE_TS
    ...
    */
    ...
    #define EMERGENTRECD_SUPPORT        (1) // 1: support emergent record to do dual writing file.
    #define SUPPORT_SHARE_REC           (1)

    #define SUPPORT_MFE_SHAREBUFFER     (0)
#if (SUPPORT_MFE_SHAREBUFFER)
    #define COMPONENT_SUPPORT_ORDER     (1)
    #define MFE_SHARE_BUF_LINE_BIAS     (32)
#else
    #define COMPONENT_SUPPORT_ORDER     (0)
    #define MFE_SHARE_BUF_LINE_BIAS     (0)
#endif
    ...
    #define EN_VR_THUMB                 (0x01)
    #define CREAT_JPG_FILE              (0x02)
    #define SUPPORT_VR_THUMBNAIL        (EN_VR_THUMB | CREAT_JPG_FILE)

    #define VIDRECD_MULTI_TRACK         (1) // multiple track for video recording
...
@{ */
    ...
    #define USB_UVC_BULK_EP             (1) // 0: Using USB ISO Tx, 1: Using USB Bulk Tx // must be 0 for Linux
    #define USING_STILL_METHOD_1        (1)
    #define USB_UVC_H264                (1) // for Logitech H264

    #if (USB_UVC_BULK_EP == 1)
    #define UVC_DMA_SIZE                (0xC000)
...
@{ */
    ...
    #define DEFAULT_ADC_DIGITAL_GAIN    (30) //unit:dB, 1dB/step  (0x0~0x30) 0dB ~30dB (not support negative dB here, HW support -64dB~64dB 0.5dB/step)
    #define DEFAULT_ADC_ANALOG_GAIN     (7)  //unit:index, 3db/index (0x00~0x7) -6/-3/0/3/6/9/12/15db max
    ...
    #if (MTS_MUXER_EN)
    #define SPEECH_RECOGNIZE_EN         (0) //ref:LIVE_PCM_R_EN
    #else
    #define SPEECH_RECOGNIZE_EN         (1) //ref:LIVE_PCM_R_EN
    #endif
/** @} */ // end of AUDIO
...
@{ */
    ...
    #if (SUPPORT_MDTC)
    #define MDTC_ON_SCORE               (0) // 1: run motion detection on slave core
    #else
    #define MDTC_ON_SCORE               (0)
    #endif
    #if (SUPPORT_ADAS)
    #define ADAS_ON_SCORE               (0) // 1: run LDWS on slave core
    #endif
...
void ____Global_Config____(){ruturn;} //dummy
    ...
    #define LIVE_AAC_R_EN               (0)
        ...
        #define AUDIO_MP3_R_EN          (0) // Use for some common audio MP3 record usage.
    #else
    ...
    #else
        #define VIDEO_P_EN              (1) //remove late by pohan chen
    #endif
