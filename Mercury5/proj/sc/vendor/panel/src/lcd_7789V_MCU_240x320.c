...
WriteCmd( 0xC6);
WriteData( 0x0A);//WriteData( 0x0A);   //0x09orig [DB7-5] 0forDotInv,1forColumnInv ,0xEF  蹈楹
//WriteData( 0x19); //KevinChen 20180525 modify to 45HZ
...
{
    RTNA_DBG_Str(0, "### RTNA_LCD_Init for ST7789V 240x320 ...\r\n");
    ...
    MMPS_PWM_Initialize();
    MMPS_PWM_SetOutputPin(MMP_PWM0_PIN, DRV_PWM_PM_PAD_MODE2);
    MMPS_PWM_OutputPulse(MMP_PWM0_PIN, AHC_TRUE, 20000, 0, MMP_TRUE, MMP_FALSE, NULL, 0);
...
void RTNA_LCD_Backlight(MMP_BOOL bEnable)
{
    if(bEnable)
    {
        MMPS_PWM_Initialize();
        MMPS_PWM_SetOutputPin(MMP_PWM0_PIN, DRV_PWM_PM_PAD_MODE2);
        MMPS_PWM_OutputPulse(MMP_PWM0_PIN, AHC_TRUE, 20000, 80, MMP_TRUE, MMP_FALSE, NULL, 0);
    }
    else
    {
        MMPS_PWM_Initialize();
