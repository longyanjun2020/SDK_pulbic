...
void RTNA_LCD_InitMainLCD(void)
{
    ...
    m_PanelAttr.ulDotClkFreq 	= 28000000;//LCD_RGB565
	m_PanelAttr.ubBusCfg		= LCD_RGB565;//LCD_TYPE_RGBLCD & LCD_TYPE_CCIR601 need to setting only.
	m_PanelAttr.ubRgbOrder		= LCD_RGB_ORDER_RGB;//LCD_TYPE_PLCD  & LCD_TYPE_PLCD_FLM need to setting only.

    // RGB interface
    m_PanelAttr.usHBPorch 		= 46;
    m_PanelAttr.usHBlanking 	= 256;
    m_PanelAttr.usHSyncW 		= 3;
    m_PanelAttr.usVBPorch 		= 27;
    m_PanelAttr.usVBlanking 	= 45;
    m_PanelAttr.usVSyncW 		= 3;
    m_PanelAttr.usPRT2HdotClk 	= 2;

    m_PanelAttr.ubDclkPor       = 1;
