...
BitStream *BitStream_new(void)
{
	...
	bstream = (BitStream *)QRcode_malloc(sizeof(BitStream));
...
static int BitStream_allocate(BitStream *bstream, int length)
{
	...
	data = (unsigned char *)QRcode_malloc(length);
	if(data == NULL) {
		return -1;
	}

	if(bstream->data) {
		QRcode_memfree(bstream->data);
	}
...
int BitStream_append(BitStream *bstream, BitStream *arg)
{
	...
	data = (unsigned char *)QRcode_malloc(bstream->length + arg->length);
	if(data == NULL) {
		return -1;
	}
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	data = (unsigned char *)QRcode_malloc((size + 7) / 8);
	if(data == NULL) {
		return NULL;
	}
