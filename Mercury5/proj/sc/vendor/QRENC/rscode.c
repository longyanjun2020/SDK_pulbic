...
//#define NULL ((void *)0)
  ...
  rs = (RS *)QRcode_calloc(1,sizeof(RS));
  if(rs == NULL)
    goto done;
  ...
  rs->alpha_to = (data_t *)QRcode_malloc(sizeof(data_t)*(rs->nn+1));
  if(rs->alpha_to == NULL){
    QRcode_memfree(rs);
    rs = NULL;
    goto done;
  }
  rs->index_of = (data_t *)QRcode_malloc(sizeof(data_t)*(rs->nn+1));
  if(rs->index_of == NULL){
    QRcode_memfree(rs->alpha_to);
    QRcode_memfree(rs);
    rs = NULL;
    goto done;
  }
  ...
  rs->genpoly = (data_t *)QRcode_malloc(sizeof(data_t)*(nroots+1));
  if(rs->genpoly == NULL){
    QRcode_memfree(rs->alpha_to);
    QRcode_memfree(rs->index_of);
    QRcode_memfree(rs);
