...
extern void *QRcode_malloc(unsigned int size);
...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)QRcode_malloc(sizeof(QRinput_List));
	if(entry == NULL) return NULL;

	entry->mode = mode;
	entry->size = size;
	if(size > 0) {
		entry->data = (unsigned char *)QRcode_malloc(size);
		if(entry->data == NULL) {
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)QRcode_malloc(sizeof(QRinput_List));
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)QRcode_malloc(n->size);
	if(n->data == NULL) {
		QRcode_memfree(n);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	input = (QRinput *)QRcode_malloc(sizeof(QRinput));
...
static int QRinput_appendPaddingBit(BitStream *bstream, QRinput *input)
{
	...
	if(padlen > 0) {
		padbuf = (unsigned char *)QRcode_malloc(padlen);
		if(padbuf == NULL) {
			ret = -1;
			goto DONE;
		}
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)QRcode_malloc(sizeof(QRinput_InputList));
	if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)QRcode_malloc(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)QRcode_malloc(bytes);
