...
#define QR_CODE_DIS_OFFSET_X	16
#define QR_CODE_DIS_OFFSET_X	16
#define	QRX	QR_CODE_DIS_OFFSET_X
#define QRY QR_CODE_DIS_OFFSET_X
#define	QRBX 10				//x offset of BORDER
#define QRBY 10				//y offset of BORDER

#define QRD						4//Pixel per dot

void *QRcode_malloc(unsigned int size);
extern void QRcode_memfree(void* addr);
extern void *QRcode_calloc(unsigned int num,unsigned int size);
...
MMP_ULONG g_ulQREncodeAddrStart;
...
__STATIC QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw = (QRRawCode *)QRcode_malloc(sizeof(QRRawCode));
	...
	raw->ecccode = (unsigned char *)QRcode_malloc(raw->eccLength);
	if(raw->ecccode == NULL) {
		QRcode_memfree(raw->datacode);
		QRcode_memfree(raw);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)QRcode_calloc(raw->blocks, sizeof(RSblock));
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
...
__STATIC void QRraw_free(QRRawCode *raw)
{
	if(raw != NULL) {
		QRcode_memfree(raw->datacode);
		QRcode_memfree(raw->ecccode);
		QRcode_memfree(raw->rsblock);
		QRcode_memfree(raw);
	}
}
...
__STATIC MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw = (MQRRawCode *)QRcode_malloc(sizeof(MQRRawCode));
	if(raw == NULL) return NULL;
	...
	raw->rsblock = (RSblock *)QRcode_calloc(1, sizeof(RSblock));
	if(raw->rsblock == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
static FrameFiller *FrameFiller_new(int width, unsigned char *frame, int mqr)
{
	...
	filler = (FrameFiller *)QRcode_malloc(sizeof(FrameFiller));
...
__STATIC QRcode *QRcode_new(int version, int width, unsigned char *data)
{
	...
	qrcode = (QRcode *)QRcode_malloc(sizeof(QRcode));
...
{
	if(qrcode != NULL) {
		QRcode_memfree(qrcode->data);
		QRcode_memfree(qrcode);
	}
}
...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	if(mask == -2) { // just for debug purpose
		masked = (unsigned char *)QRcode_malloc(width * width);
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)QRcode_malloc(sizeof(QRcode_List));
...
void Disp_FixelPic(UINT16 uwDisplayID,int iPenX,int iPenY,PT_PixelDatas ptPixelDatas)
{    
	int i=0,j=0;
	
	//QR background,white
	#ifdef ENABLE_GUI_SUPPORT_MULTITASK
	AHC_OSDSetColor(uwDisplayID, OSD_COLOR_WHITE);
	#else
	AHC_OSDSetColor(OSD_COLOR_WHITE);
	#endif
	AHC_OSDDrawFillRect(uwDisplayID,QRBX, QRBY,(QRBX+ptPixelDatas->iWidth*QRD+2*iPenX), (QRBY+ptPixelDatas->iHeight*QRD+2*iPenY));
	...
	for(i=0;i<ptPixelDatas->iHeight;i++)	
	{
		for(j=0;j<ptPixelDatas->iWidth;j++)		
		{
			...
			{
				#ifdef ENABLE_GUI_SUPPORT_MULTITASK
				AHC_OSDSetColor(uwDisplayID, OSD_COLOR_DARKGRAY3);
				#else
				AHC_OSDSetColor(OSD_COLOR_DARKGRAY3);
				#endif
				AHC_OSDDrawFillRect(uwDisplayID,(QRBX+j*QRD+iPenX), (QRBY+i*QRD+iPenY),(QRBX+j*QRD+QRD+iPenX), (QRBY+i*QRD+QRD+iPenY));
			}	
			else
			{
				#ifdef ENABLE_GUI_SUPPORT_MULTITASK
...
int Fb_QrDisp(UINT16 uwDisplayID,int iPenX,int iPenY,QRcode*pQRcode)
{         
	...
	BEGIN_LAYER(uwDisplayID);
	Disp_FixelPic(uwDisplayID,iPenX,iPenY,&g_tOriginPixelDatas);
	END_LAYER(uwDisplayID);
...
{
	unsigned int addr;
	addr = (g_ulQREncodeAddrStart+3)&0xfffffffc;
	g_ulQREncodeAddrStart = (addr + size);
	memset((void*)addr,0,size);
	
    if (g_ulQREncodeAddrStart > MMPS_System_GetMemHeapEnd()) {
        printc("\t= [HeapMemErr] UVC rear MJPG Video preview =\r\n");
        //return MMP_3GPRECD_ERR_MEM_EXHAUSTED;
    }
    printc("End of QRCODE addr = 0x%X\r\n", g_ulQREncodeAddrStart);
	return (void*)addr;
}

void *QRcode_calloc(unsigned int num,unsigned int size)
{
	unsigned int addr;
	unsigned int ttsize;
	ttsize = num*size;
	...
	addr = (g_ulQREncodeAddrStart+3)&0xfffffffc;
	g_ulQREncodeAddrStart = (addr + ttsize);
	
    if (g_ulQREncodeAddrStart > MMPS_System_GetMemHeapEnd()) {
        printc("\t= [HeapMemErr2] UVC rear MJPG Video preview =\r\n");
        //return MMP_3GPRECD_ERR_MEM_EXHAUSTED;
    }
    printc("QRcode_calloc addr = 0x%X\r\n", g_ulQREncodeAddrStart);
	return (void*)addr;
}

void QRcode_memfree(void* addr)
{
    printc("QRcode_free addr = 0x%X\r\n", addr);
	addr = NULL;
}

void test_encodeTooLongMQR(UINT16 uwDisplayID)
{
	QRcode *code;
	...
	printc("%s::Encode too large data for MQR.\r\n", __func__);
	
	printc(FG_BLUE("@@@ Dram Memory End Addr of Video Record: 0x%X")"\r\n", ulVRDramEnd);
	g_ulQREncodeAddrStart = ulVRDramEnd;
	
	code = QRcode_encodeString8bitMQR(data[0], 4, QR_ECLEVEL_L);
	if(NULL == code)
		printc("%s::16 byte length 8bit string was accepted to version 4.\r\n", __func__);

	if(code != NULL) {
		Fb_QrDisp(uwDisplayID,QRX,QRY,code);
		printQRcode(code);
		QRcode_free(code);
	}
}

void QR_encodeTooLong(UINT16 uwDisplayID,const char *pwd)
{
	static QRcode *code = NULL;
	MMP_ERR sRet = MMP_ERR_NONE;
	MMP_ULONG ulVRDramEnd;
	int version;
	static char data[64] = "This is test.";//MAX_PATH_LEN

	if(strcmp(data,pwd))/*(NULL == code)*/{
		strcpy(data,pwd);
		
		printc(FG_BLUE("@@@ Dram Memory End Addr of Video Record: 0x%X")"\r\n", ulVRDramEnd);
		
		for(version = 4; version < 5; version++) {//1~11version for test,8KB memory will be used.
			g_ulQREncodeAddrStart = ulVRDramEnd;
			code = QRcode_encodeString(pwd, version, QR_ECLEVEL_L, QR_MODE_8, 0);
			if(code == NULL) {
				printc("%s::Failed to encode\r\n", __func__);
			} else {
				Fb_QrDisp(uwDisplayID,QRX,QRY,code);
				//QRcode_free(code);
				QRcode_clearCache();
	...
	//	for(version = 31; version < 41; version++) {//31~41 version
	//		g_ulQREncodeAddrStart = ulVRDramEnd;
