...
MMP_ERR Gsensor_Initial(struct _3RD_PARTY_GSENSOR *pthis, GSNR_ATTRIBUTE *pgsnr_attr)
{
    ...
    {	
        ...
        pthis->MMPF_Gsensor_SetReg(pthis, GSensor_Reg_MODE, 0x00);
        pthis->MMPF_Gsensor_SetReg(pthis, GSensor_Reg_SPCNT, 0x00);
        pthis->MMPF_Gsensor_SetReg(pthis, GSensor_Reg_INTSU, 0x03); // No mapping to INT_x
	 ...
	 pthis->MMPF_Gsensor_SetReg(pthis, GSensor_Reg_PD, 0xE0);
        pthis->MMPF_Gsensor_SetReg(pthis, GSensor_Reg_MODE, 0x41); // INT high active


        //pthis->MMPF_Gsensor_SetDynamicRange(pthis, pgsnr_attr, pgsnr_attr->ubRange);

        pthis->MMPF_Gsensor_GetReg(pthis, GSensor_Reg_INTSU, &u8temp);
	 printc("GSensor_Reg_INTSU:%x\r\n",u8temp);
        pthis->MMPF_Gsensor_GetReg(pthis, GSensor_Reg_PDET, &u8temp);
	 printc("GSensor_Reg_PDET:%x\r\n",u8temp);
	 pthis->MMPF_Gsensor_GetReg(pthis, GSensor_Reg_SR, &u8temp);
	 printc("GSensor_Reg_SR:%x\r\n",u8temp);
	 pthis->MMPF_Gsensor_GetReg(pthis, GSensor_Reg_PD, &u8temp);
	 printc("GSensor_Reg_PD:%x\r\n",u8temp);
	 pthis->MMPF_Gsensor_GetReg(pthis, GSensor_Reg_MODE, &u8temp);
	 printc("GSensor_Reg_MODE:%x\r\n",u8temp);
