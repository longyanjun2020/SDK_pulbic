...
#define STORAGE_ERASE_ERROR					-17 // storage can't be erased.
...
typedef struct  
{
	u32 	nb_cylinder; // there are nb_head tracks per cylinder
  	u32   nb_head;
	u32   	nb_sector;	// nb sectors per tracks
	u32   	nb_sectors_per_cluster;	// nb sectors per cluster, for media with fixed cluster size
